...
const TEST_ONBOARDING = process.env.WEBFORGE_TEST_ONBOARDING === "1"
...
const main = Effect.gen(function* () {
  ...
  process.env.WEBFORGE_DISABLE_EMBEDDED_WEB_UI = "true"

  const appId = app.isPackaged ? APP_IDS[CHANNEL] : "ai.webforge.desktop.dev"
  const onboardingTestRoot = ((): string | undefined => {
    if (!TEST_ONBOARDING) return

    const root = join(tmpdir(), `webforge-onboarding-${randomUUID()}`)
  ...
  app.setName(app.isPackaged ? APP_NAMES[CHANNEL] : "WebForge Dev")
  ...
  app.on("second-instance", (_event: Event, argv: string[]) => {
    const urls = argv.filter((arg: string) => arg.startsWith("webforge://"))
    if (urls.length) {
  ...
  app.setAsDefaultProtocolClient("webforge")
  registerRendererProtocol()
  setDockIcon()
  ...
  const port = yield* Effect.gen(function* () {
    const fromEnv = process.env.WEBFORGE_PORT
    if (fromEnv) {
