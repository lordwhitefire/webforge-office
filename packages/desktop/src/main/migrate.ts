...
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.webforge.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `webforge.settings.dat` is special: it maps to the `webforge.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"webforge.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // webforge.settings.dat → the electron settings store ("webforge.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "webforge.global.dat").
  const storeName = filename === "webforge.settings.dat" ? "webforge.settings" : filename
