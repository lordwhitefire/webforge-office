...
import { type WslCommandLine, resolveWslWebforge, shellEscape, wslArgs } from "./runtime"
...
export async function spawnWslSidecar(
  ...
  const webforge = await resolveWslWebforge(distro)
  if (!webforge) throw new Error(`WebForge is not installed in ${distro}`)

  const port = await allocatePort()
  const password = randomUUID()
  const username = "webforge"
  const script = [
    "set -euo pipefail",
    'cd "$HOME" || cd /',
    ...
    "export PATH",
    "export WSLENV=",
    "export WEBFORGE_EXPERIMENTAL_DISABLE_FILEWATCHER=true",
    "export WEBFORGE_CLIENT=desktop",
    `export WEBFORGE_SERVER_USERNAME=${shellEscape(username)}`,
    `export WEBFORGE_SERVER_PASSWORD=${shellEscape(password)}`,
    'export XDG_STATE_HOME="$HOME/.local/state"',
    `exec ${shellEscape(webforge)} --print-logs --log-level ${app.isPackaged ? "WARN" : "INFO"} serve --hostname 0.0.0.0 --port ${port}`,
  ].join("\n")
