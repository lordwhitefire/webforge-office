...
import {
  ...
  useCommand,
  useWslServers,
} from "@webforge-ai/app"
import type { UpdaterState } from "@webforge-ai/app/updater"
    ...
    integrations: (integrations) => {
      return integrations.filter(
        (i) =>
          i.name !== "Breadcrumbs" &&
          !(
            import.meta.env.WEBFORGE_CHANNEL === "prod" &&
...
const deepLinkEvent = "webforge:deep-link"
...
const emitDeepLinks = (urls: string[]) => {
  ...
  window.__WEBFORGE__ ??= {}
  const pending = window.__WEBFORGE__.deepLinks ?? []
  window.__WEBFORGE__.deepLinks = [...pending, ...urls]
...
function windowLastActiveUrlKey(windowID: string) {
  return `webforge.desktop.window.${windowID}.last-active-url`
}
...
function DesktopRoot(props: { windowState: DesktopWindowState }) {
  ...
  const loadLocale = async () => {
    const current = await platform.storage?.("webforge.global.dat").getItem("language")
