...
describe("inference stat normalization", () => {
  ...
  test("uses provider.model to resolve webforge route providers", () => {
    ...
    expect(statProvider("big-pickle", "claude-sonnet-4-5", "webforge")).toBe("anthropic")
    expect(statProvider("big-pickle", "gpt-5", "webforge")).toBe("openai")
    expect(statProvider("big-pickle", "", "webforge")).toBe("unknown")
  ...
  test("model aggregates prefer provider.model and use normalized model", () => {
    ...
    expect(
      toModelAggregate({ ...aggregate("big-pickle", "webforge"), provider_model: "claude-sonnet-4-5" }),
    ).toMatchObject([
      {
        provider: "anthropic",
  ...
  test("provider aggregates never keep webforge as the provider", () => {
    expect(toProviderAggregate({ ...aggregate("big-pickle", "webforge"), provider_model: "gpt-5" })).toMatchObject([
      { provider: "openai" },
    ])
    expect(toProviderAggregate(aggregate("big-pickle", "webforge"))).toMatchObject([{ provider: "unknown" }])
  })

  test("geo aggregates never keep webforge or big-pickle dimensions", () => {
    expect(toGeoAggregate({ ...aggregate("big-pickle", "webforge"), country: "US" })).toMatchObject([
