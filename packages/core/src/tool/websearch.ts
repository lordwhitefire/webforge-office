...
import { ToolFailure } from "@webforge-ai/llm"
...
export class ConfigService extends Context.Service<ConfigService, Config>()("@webforge/v2/WebSearchConfig") {}

/** Isolates the retained product environment contract from the generic tool implementation. */
export const defaultConfigLayer = Layer.sync(ConfigService, () =>
  ConfigService.of({
    provider:
      process.env.WEBFORGE_WEBSEARCH_PROVIDER === "exa" || process.env.WEBFORGE_WEBSEARCH_PROVIDER === "parallel"
        ? process.env.WEBFORGE_WEBSEARCH_PROVIDER
        : undefined,
    enableExa: truthy("WEBFORGE_EXPERIMENTAL") || truthy("WEBFORGE_ENABLE_EXA") || truthy("WEBFORGE_EXPERIMENTAL_EXA"),
    enableParallel: truthy("WEBFORGE_ENABLE_PARALLEL") || truthy("WEBFORGE_EXPERIMENTAL_PARALLEL"),
              ...
              const text =
                provider === "exa"
                  ...
                  : yield* callMcp(
                      ...
                      {
                        "User-Agent": `webforge/${InstallationVersion}`,
