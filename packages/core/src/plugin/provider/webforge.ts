...
import type { IntegrationOAuthMethodRegistration } from "@webforge-ai/plugin/v2/effect/integration"
import { define } from "@webforge-ai/plugin/v2/effect/plugin"
import type { CredentialValue } from "@webforge-ai/sdk/v2/types"
...
const defaultServer = "https://console.webforge.ai"
const clientID = "webforge-cli"
...
export const WebforgePlugin = define<HttpClient.HttpClient | EventV2.Service | Scope.Scope>({
  id: "webforge",
  effect: Effect.fn(function* (ctx) {
    ...
    const load = Effect.fn("WebforgePlugin.load")(function* () {
      const connection = yield* ctx.integration.connection.active("webforge")
      ...
      providers = credential
        ? yield* fetchProviders(http, credential).pipe(
            Effect.catch((cause) =>
              Effect.logWarning("failed to load WebForge provider config", { cause }).pipe(Effect.as(undefined)),
            ),
          )
        : undefined
    })
    ...
    yield* ctx.integration.transform((draft) => {
      draft.update("webforge", (integration) => {
        integration.name = "WebForge"
      })
      draft.method.update(oauth(http))
      draft.method.update({ integrationID: "webforge", method: { type: "key", label: "API key (service account)" } })
    })

    connected = (yield* ctx.integration.connection.active("webforge")) !== undefined
    yield* ctx.catalog.transform((catalog) => {
      for (const [providerID, item] of Object.entries(providers ?? {})) {
        catalog.provider.update(providerID, (provider) => {
          provider.integrationID = Integration.ID.make("webforge")
      ...
      const item = catalog.provider.get(ProviderV2.ID.webforge)
      if (!item) return
      const hasKey = Boolean(process.env.WEBFORGE_API_KEY || connected || item.provider.request.body.apiKey)
    ...
    yield* events.subscribe(Integration.Event.ConnectionUpdated).pipe(
      Stream.filter((event) => event.data.integrationID === Integration.ID.make("webforge")),
