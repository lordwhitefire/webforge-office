...
describe("Npm.sanitize", () => {
  test("keeps normal scoped package specs unchanged", () => {
    expect(Npm.sanitize("@webforge/acme")).toBe("@webforge/acme")
    expect(Npm.sanitize("@webforge/acme@1.0.0")).toBe("@webforge/acme@1.0.0")
    expect(Npm.sanitize("prettier")).toBe("prettier")
  })

  test("handles git https specs", () => {
    const spec = "acme@git+https://github.com/webforge/acme.git"
    const expected = win ? "acme@git+https_//github.com/webforge/acme.git" : spec
