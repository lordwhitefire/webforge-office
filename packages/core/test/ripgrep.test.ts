...
import { RelativePath } from "@webforge-ai/core/schema"
...
describe("Ripgrep", () => {
  ...
  it.live("never includes git metadata", () =>
    Effect.acquireUseRelease(
      ...
      (tmp) =>
        Effect.gen(function* () {
          yield* Effect.promise(() => fs.mkdir(path.join(tmp.path, ".webforge")))
          yield* Effect.promise(() => fs.writeFile(path.join(tmp.path, ".webforge", "config"), "needle\n"))
          ...
          expect(files.map((item) => item.path)).toContain(RelativePath.make(".webforge/config"))
          ...
          expect(matches.map((item) => item.entry.path)).toContain(RelativePath.make(".webforge/config"))
