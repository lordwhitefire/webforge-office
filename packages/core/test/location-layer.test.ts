...
import { define } from "@webforge-ai/plugin/v2/effect"
...
import { LocationServiceMap } from "@webforge-ai/core/location-services"
      ...
      Effect.flatMap(([blocked, allowed]) =>
        Effect.gen(function* () {
          ...
          yield* Effect.promise(() =>
            fs.writeFile(
              path.join(blocked.path, "webforge.json"),
              JSON.stringify({
    ...
    Effect.acquireRelease(
      ...
      Effect.flatMap((dir) =>
        Effect.gen(function* () {
          ...
          yield* Effect.promise(() =>
            fs.writeFile(
              path.join(dir.path, "webforge.json"),
              JSON.stringify({
                providers: {
                  unavailable: {
                    name: "Unavailable",
