...
function clearHighlightFind() {
  ...
  if (!api) return
  api.delete("webforge-find")
  api.delete("webforge-find-current")
}
...
export function createFileFind(opts: CreateFileFindOptions) {
  ...
  const setHighlights = (ranges: Range[], currentIndex: number) => {
    ...
    if (active) api.set("webforge-find-current", new Highlight(active))

    const rest = ranges.filter((_, i) => i !== currentIndex)
    if (rest.length > 0) api.set("webforge-find", new Highlight(...rest))
    return true
  }
