...
describe("inlineCodeKind", () => {
  test("leaves code expressions as normal inline code", () => {
    ...
    expect(inlineCodeKind(`@webforge-ai/app <StatusPopover />)`)).toBeUndefined()
  ...
  test("detects file and directory paths", () => {
    ...
    expect(inlineCodeKind(`~/.config/webforge`)).toBe("path")
    expect(inlineCodeKind(`@webforge-ai/app`)).toBe("path")
  ...
  test("detects urls", () => {
    ...
    expect(inlineCodeKind(`file:///tmp/webforge`)).toBeUndefined()
