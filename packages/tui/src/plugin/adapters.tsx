import type { TuiDialogSelectOption, TuiPluginApi, TuiSlotProps } from "@webforge-ai/plugin/tui"
...
import type { useWebforgeKeymap } from "../keymap"
...
type Input = {
  ...
  keymap: ReturnType<typeof useWebforgeKeymap>
...
export function createTuiApiAdapters(input: Input): Omit<TuiPluginApi, "lifecycle"> {
  return {
    ...
    mode: {
      current() {
        return Keymap.getWebforgeModeStack(input.keymap).current()
      },
      push(mode) {
        return Keymap.getWebforgeModeStack(input.keymap).push(mode)
      },
    },
    route: {
      register(list) {
