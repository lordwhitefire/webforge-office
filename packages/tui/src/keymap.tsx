...
export const WEBFORGE_BASE_MODE = "base"
export const COMMAND_PALETTE_COMMAND = "command.palette.show"

const WEBFORGE_MODE_KEY = "webforge.mode"

export const WebforgeKeymapProvider = KeymapProvider
export const useWebforgeKeymap = useKeymap

export { useBindings, useKeymapSelector }

export type OpenTuiKeymap = ReturnType<typeof useKeymap>
type WebforgeModeStack = ReturnType<typeof createWebforgeModeStack>
...
const modeStacks = new WeakMap<OpenTuiKeymap, WebforgeModeStack>()

function isVisiblePaletteCommand(command: Command) {
  return command.hidden !== true && command.name !== COMMAND_PALETTE_COMMAND
}

export function createWebforgeModeStack(keymap: OpenTuiKeymap) {
  keymap.setData(WEBFORGE_MODE_KEY, WEBFORGE_BASE_MODE)

  const offFields = keymap.registerLayerFields({
    mode(value, ctx) {
      ctx.require(WEBFORGE_MODE_KEY, value)
    },
  })
  ...
  const update = () => {
    keymap.setData(WEBFORGE_MODE_KEY, stack.at(-1)?.mode ?? WEBFORGE_BASE_MODE)
  }

  const stackApi = {
    current() {
      return stack.at(-1)?.mode ?? WEBFORGE_BASE_MODE
    },
    push(mode: string) {
      if (disposed) return () => {}
    ...
    dispose() {
      ...
      stack.length = 0
      offFields()
      keymap.setData(WEBFORGE_MODE_KEY, undefined)
...
export function useWebforgeModeStack() {
  return getWebforgeModeStack(useWebforgeKeymap())
}

export function getWebforgeModeStack(keymap: OpenTuiKeymap) {
  const value = modeStacks.get(keymap)
  if (!value) throw new Error("Webforge mode stack is not registered for this keymap")
  return value
}
...
export function registerWebforgeKeymap(keymap: OpenTuiKeymap, renderer: CliRenderer, config: ResolvedKeymapConfig) {
  const modeStack = createWebforgeModeStack(keymap)
...
export function useCommandSlashes(): Accessor<readonly CommandSlashEntry[]> {
  const keymap = useWebforgeKeymap()
