...
Ajan becerileri, webforge'un deponuzdan veya ev dizininizden tekrar kullanilabilir talimatlari kesfetmesini saglar.
...
webforge su konumlari tarar:

- Proje konfigurasyonu: `.webforge/skills/<name>/SKILL.md`
- Genel konfigurasyon: `~/.config/webforge/skills/<name>/SKILL.md`
...
Proje yerel yollarinda webforge, mevcut calisma dizininizden baslayip git worktree kokune kadar yukari cikar.
Bu sirada `skills/*/SKILL.md` altindaki eslesen `.webforge/` dosyalarini ve `.claude/skills/*/SKILL.md` ya da `.agents/skills/*/SKILL.md` dosyalarini yukler.

Genel tanimlar da `~/.config/webforge/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` ve `~/.agents/skills/*/SKILL.md` yollarindan yuklenir.
...
`.webforge/skills/git-release/SKILL.md` dosyasini soyle olusturun:
...
webforge, `skill` araci aciklamasinda mevcut becerileri listeler.
...
`webforge.json` icinde pattern tabanli izinlerle ajanlarin hangi becerilere erisecegini kontrol edin:
...
**Yerlesik ajanlar icin** (`webforge.json` icinde):
