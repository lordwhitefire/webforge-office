---
title: Eklentiler
description: WebForge'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek WebForge'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya WebForge'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.webforge/plugins/` - Proje düzeyinde eklentiler
- `~/.config/webforge/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/webforge/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/webforge/webforge.json`)
2. Project config (`webforge.json`)
3. Global eklenti dizini (`~/.config/webforge/plugins/`)
4. Proje eklenti dizini (`.webforge/plugins/`)
...
WebForge bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir WebForge SDK istemcisi.
...
WebForge'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
WebForge masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
WebForge'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca WebForge'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, WebForge'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra WebForge'a da hazır olacaktır.
