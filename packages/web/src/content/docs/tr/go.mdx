...
WebForge Go, popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir aboneliktir — **ilk ayınız için 5$**, sonrasında **aylık 10$**.

Go, WebForge'daki diğer sağlayıcılar gibi çalışır. WebForge Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve WebForge'u kullanmak için bunu kullanmanıza gerek yoktur.
...
WebForge Go, bu modellere **ilk ayınız için 5$**, ardından **aylık 10$** karşılığında erişmenizi sağlar.

---

## Nasıl çalışır?

WebForge Go, WebForge'daki diğer sağlayıcılar gibi çalışır.

1. **<a href={console}>WebForge Zen</a>**'e giriş yapın, Go'ya abone olun ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, `WebForge Go`yu seçin ve API anahtarınızı yapıştırın.
3. Go üzerinden kullanabileceğiniz modellerin listesini görmek için TUI'de `/models` komutunu çalıştırın.

:::note
Her çalışma alanından yalnızca bir üye WebForge Go'ya abone olabilir.
:::
...
WebForge Go aşağıdaki limitleri içerir:
...
- Kimi K2.7 Code/K2.6 — İstek başına 870 girdi, 55.000 önbelleğe alınmış, 200 çıktı token'ı
...
WebForge yapılandırmanızdaki [model id](/docs/config/#models) formatı `webforge-go/<model-id>` şeklindedir. Örneğin, Kimi K2.7 Code için yapılandırmanızda `webforge-go/kimi-k2.7-code` kullanmalısınız.
...
WebForge Go'yu şu amaçlarla oluşturduk:
...
4. WebForge ile diğer sağlayıcıları da kullanmanıza olanak tanıyarak **kilitlenme (vendor lock-in) olmamasını** sağlamak.
