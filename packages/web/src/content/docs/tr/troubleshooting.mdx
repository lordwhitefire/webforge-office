...
webforge ile ilgili bir sorunu ayiklamak icin once loglari ve diskte tuttugu yerel verileri kontrol edin.
...
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.local\share\webforge\log` yapistirin

Log dosyalari zaman damgasiyla adlandirilir (ornegin `2025-01-09T123456.log`) ve en yeni 10 log dosyasi tutulur.

Daha ayrintili ayiklama bilgisi icin `--log-level` komut satiri secenegini kullanabilirsiniz. Ornek: `webforge --log-level DEBUG`.

---

## Depolama

webforge, oturum verilerini ve diger uygulama verilerini diskte su konumda saklar:

- **macOS/Linux**: `~/.local/share/webforge/`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.local\share\webforge` yapistirin
...
webforge Desktop arka planda yerel bir webforge sunucusu (`webforge-cli` sidecar) calistirir. Sorunlarin cogu bozuk bir plugin, hasarli cache veya hatali sunucu ayarindan kaynaklanir.

### Hızlı kontroller

- Uygulamayi tamamen kapatip yeniden acin
- Uygulama hata ekrani gosteriyorsa **Restart**'a tiklayip hata detaylarini kopyalayin
- Yalnizca macOS: `webforge` menusu -> **Reload Webview** (arayuz bos/donukse yardimci olur)
...
- **macOS/Linux**: `~/.config/webforge/webforge.jsonc` (veya `~/.config/webforge/webforge.json`)
- **macOS/Linux** (eski kurulumlar): `~/.local/share/webforge/webforge.jsonc`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\webforge\webforge.jsonc` yapistirin
...
webforge diskten yerel plugin de yukleyebilir. Bu dizinleri gecici olarak tasiyin (veya klasoru yeniden adlandirin) ve uygulamayi yeniden baslatin:

- **Global plugins**
  - **macOS/Linux**: `~/.config/webforge/plugins/`
  - **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\webforge\plugins` yapistirin
...
Pluginleri kapatmak ise yaramazsa (veya plugin kurulumu takili kaldiysa), webforge'un cache'i yeniden olusturmasi icin cache'i temizleyin.

1. webforge Desktop'u tamamen kapatin
2. Cache dizinini silin:

- **macOS**: Finder -> `Cmd+Shift+G` -> `~/.cache/webforge`
- **Linux**: `~/.cache/webforge` dizinini silin (veya `rm -rf ~/.cache/webforge` calistirin)
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.cache\webforge` yapistirin

3. webforge Desktop'u yeniden baslatin

---

### Sunucu bağlantı sorunlarını düzeltin

webforge Desktop ya kendi yerel sunucusunu baslatir (varsayilan) ya da sizin tanimladiginiz bir sunucu URL'sine baglanir.
...
`webforge.json(c)` dosyanizda `server` bolumu varsa gecici olarak kaldirin ve desktop uygulamasini yeniden baslatin.

#### Ortam degiskenlerini kontrol edin

Ortamda `WEBFORGE_PORT` ayarliysa desktop uygulamasi yerel sunucu icin o portu kullanmaya calisir.

- `WEBFORGE_PORT` degerini kaldirin (veya bos bir port secin) ve yeniden baslatin
...
Windows'ta webforge Desktop, Microsoft Edge **WebView2 Runtime** gerektirir. Uygulama bos pencereyle aciliyorsa veya hic baslamiyorsa WebView2'yi kurup/guncelleyip tekrar deneyin.

---

### Windows: Genel performans sorunları

Windows'ta yavaslik, dosya erisim sorunlari veya terminal problemleri yasiyorsaniz [WSL (Windows Subsystem for Linux)](/docs/windows-wsl) kullanmayi deneyin. WSL, webforge ozellikleriyle daha sorunsuz calisan bir Linux ortami saglar.

---

### Bildirimler görünmüyor

webforge Desktop sistem bildirimlerini yalnizca su durumlarda gosterir:

- Isletim sistemi ayarlarinizda webforge icin bildirimler etkinse
...
1. webforge Desktop'u kapatin
2. Su dosyalari bulun ve silin (webforge Desktop uygulama veri dizininde yer alirlar):

- `webforge.settings.dat` (desktop varsayilan sunucu URL'si)
- `webforge.global.dat` ve `webforge.workspace.*.dat` (son sunucular/projeler gibi UI durumu)
...
webforge ile ilgili bir sorun yasiyorsaniz:
...
3. `webforge upgrade` ile en guncel surumu kullandiginizdan emin olun
...
Erisiminiz olan modelleri gormek icin `webforge models` calistirin.
...
2. Sorun surerse kayitli konfigurasyonu temizlemeyi deneyin:
   ...
   Windows'ta `WIN+R` tuslayip su konumu silin: `%USERPROFILE%\.local\share\webforge`
...
API cagrisi hatalari aliyorsaniz bunun nedeni guncel olmayan provider paketleri olabilir. webforge, provider paketlerini (OpenAI, Anthropic, Google vb.) gerektikce dinamik yukler ve yerelde onbellekler.
...
1. Provider paket cache'ini temizleyin:
   ...
   Windows'ta `WIN+R` tuslayip su konumu silin: `%USERPROFILE%\.cache\webforge`

2. En guncel provider paketlerini yeniden kurmak icin webforge'u yeniden baslatin

Bu, webforge'un en yeni provider paket surumlerini indirmesini zorlar ve model parametresi/API degisikliklerinden kaynakli uyumsuzluklari cogu zaman cozer.
...
webforge, Wayland kullandiginizi algilarsa `wl-clipboard` tercihi yapar. Aksi halde sirayla `xclip` ve `xsel` araclarini arar.
