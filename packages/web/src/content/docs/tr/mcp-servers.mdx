...
_Model Bağlam Protokolü_ veya MCP kullanarak webforge'a harici araçlar ekleyebilirsiniz. webforge hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [webforge Config](https://opencode.ai/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/webforge` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
webforge, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde webforge şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa webforge, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `webforge mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından webforge, belirteçleri `~/.local/share/webforge/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra webforge'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi webforge yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve webforge'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
