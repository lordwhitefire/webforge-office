---
title: Sunucu
description: webforge sunucusuyla HTTP uzerinden etkilesin.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`webforge serve` komutu, webforge istemcisinin kullanabileceği bir OpenAPI endpoint'i açan headless bir HTTP sunucusu çalıştırır.
...
| `--mdns-domain` | mDNS hizmeti için özel alan adı                | `webforge.local` |
...
Sunucuyu HTTP temel kimlik doğrulama (basic auth) ile korumak için `WEBFORGE_SERVER_PASSWORD` ayarlayın. Kullanıcı adı varsayılan olarak `webforge` değeridir; değiştirmek isterseniz `WEBFORGE_SERVER_USERNAME` ayarlayabilirsiniz. Bu ayar hem `webforge serve` hem de `webforge web` için geçerlidir.
...
`webforge` çalıştırdığınızda hem TUI hem de sunucu başlar. TUI, sunucuyla konuşan istemci tarafıdır.
Sunucu bir OpenAPI 3.1 spesifikasyon uç noktası açar. Bu uç nokta [SDK](/docs/sdk) üretiminde de kullanılır.

:::tip
webforge ile programatik etkileşim için webforge sunucusunu kullanın.
:::

Bu mimari, webforge'un birden fazla istemciyi desteklemesini ve programatik kullanımları mümkün kılmasını sağlar.

`webforge serve` ile bağımsız bir sunucu başlatabilirsiniz. webforge TUI açıksa, `webforge serve` yeni bir sunucu başlatır.
...
[`/tui`](#tui) uç noktası sunucu üzerinden TUI'yi sürmek için kullanılabilir. Örneğin bir istemi önceden doldurabilir veya çalıştırabilirsiniz. Bu kurulum webforge [IDE](/docs/ide) eklentileri tarafından kullanılır.
...
webforge sunucusu aşağıdaki API'leri sunar.
