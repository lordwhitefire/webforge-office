...
Araçlar, LLM'in kod tabanınızda eylem gerçekleştirmesini sağlar. webforge yerleşik bir araç setiyle gelir, ancak bunu [özel araçlarla](/docs/custom-tools) veya [MCP sunucularıyla](/docs/mcp-servers) genişletebilirsiniz.
...
webforge içinde bulunan tüm yerleşik araçlar aşağıdadır.
...
:::note
Bu araç yalnızca `WEBFORGE_EXPERIMENTAL_LSP_TOOL=true` (veya `WEBFORGE_EXPERIMENTAL=true`) olduğunda kullanılabilir.
:::
...
:::note
Bu araç yalnızca webforge provider kullanılırken veya `WEBFORGE_ENABLE_EXA` ortam değişkeni doğru değerlerden birine ayarlandığında (örneğin `true` ya da `1`) kullanılabilir.

webforge başlatırken etkinleştirmek için:
