...
webforge ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak webforge kendi `webforge` temasını kullanır.
...
webforge birden fazla yerleşik temayla gelir.
...
- webforge'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
webforge, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/webforge/themes/*.json` veya `$XDG_CONFIG_HOME/webforge/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.webforge/themes/*.json`
4. **Geçerli çalışma dizini** - `./.webforge/themes/*.json`
