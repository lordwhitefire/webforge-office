...
You can add external tools to WebForge using the _Model Context Protocol_, or MCP. WebForge supports both local and remote servers.
...
You can define MCP servers in your [WebForge Config](https://opencode.ai/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Organizations can provide default MCP servers via their `.well-known/webforge` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
WebForge automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, WebForge will:
...
If the server requires authentication, WebForge will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `webforge mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, WebForge will store the tokens securely in `~/.local/share/webforge/mcp-auth.json`.
...
Your MCPs are available as tools in WebForge, alongside built-in tools. So you can manage them through the WebForge config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect WebForge to your Sentry account.
