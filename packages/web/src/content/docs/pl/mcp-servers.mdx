...
Możesz dodać dodatkowe informacje do webforge za pomocą _Model Context Protocol_, w skrócie MCP. webforge obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [webforge Config](https://opencode.ai/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/webforge`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
webforge automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, webforge:
...
Jeśli serwer wymaga uwierzytelnienia, webforge poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `webforge mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji webforge bezpieczne przechowa tokeny w `~/.local/share/webforge/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w webforge, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie webforge, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć webforge z kontem Sentry.
