...
Aby debugować problemy z webforge, zacznij od sprawdzenia dzienników i danych lokalnych przechowywanych na dysku.
...
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\webforge\log`
...
Możesz ustawić poziom dziennika za pomocą opcji wiersza poleceń `--log-level`, aby uzyskać bardziej szczegółowe informacje debugowania. Na przykład `webforge --log-level DEBUG`.

---

## Przechowywanie danych

webforge przechowuje dane sesji i inne dane aplikacji na dysku pod adresem:

- **macOS/Linux**: `~/.local/share/webforge/`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\webforge`
...
webforge Desktop uruchamia w tle lokalny serwer webforge (sidecar `webforge-cli`). Większość problemów jest spowodowana nieprawidłowo działającą wtyczką, uszkodzoną pamięcią podręczną lub złymi ustawieniami serwera.
...
- Tylko macOS: menu `WebForge` -> **Załaduj ponownie przeglądarkę internetową** (pomaga, jeśli interfejs użytkownika jest pusty/zawieszony).
...
- **macOS/Linux**: `~/.config/webforge/webforge.jsonc` (lub `~/.config/webforge/webforge.json`)
- **macOS/Linux** (starsze instalacje): `~/.local/share/webforge/webforge.jsonc`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\webforge\webforge.jsonc`
...
webforge może także ładować lokalne wtyczki z dysku. Tymczasowo usuń je (lub zmień nazwę folderu) i uruchom ponownie aplikację komputerową:

- **Wtyczki globalne**
  - **macOS/Linux**: `~/.config/webforge/plugins/`
  - **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\webforge\plugins`
...
Jeśli wyłączenie wtyczek nie pomoże (lub instalacja wtyczki utknęła), wyczyść pamięć podręczną, aby webforge mógł ją odbudować.

1. Całkowicie zamknij webforge Desktop.
2. Usuń katalog pamięci podręcznej:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/webforge`
- **Linux**: usuń `~/.cache/webforge` (lub uruchom `rm -rf ~/.cache/webforge`)
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.cache\webforge`

3. Uruchom ponownie webforge Desktop.

---

### Napraw problemy z połączeniem z serwerem

webforge Desktop może uruchomić własny serwer lokalny (domyślnie) lub połączyć się ze skonfigurowanym adresem URL serwera.
...
Jeśli Twój `webforge.json(c)` zawiera sekcję `server`, tymczasowo usuń ją i uruchom ponownie aplikację komputerową.

#### Sprawdź zmienne środowiskowe

Jeśli w swoim środowisku masz ustawiony `WEBFORGE_PORT`, aplikacja komputerowa spróbuje użyć tego portu dla serwera lokalnego.

- usuń `WEBFORGE_PORT` (lub wybierz wolny port) i uruchom ponownie.
...
W systemie Windows webforge Desktop wymaga Microsoft Edge **WebView2 Runtime**. Jeśli aplikacja otwiera się w pustym oknie lub nie uruchamia się, zainstaluj/zaktualizuj WebView2 i spróbuj ponownie.

---

### Windows: Ogólne problemy z wydajnością

Jeśli doświadczasz niskiej wydajności, problemów z dostępem do plików lub problemów z terminalem w systemie Windows, spróbuj użyć [WSL (podsystem Windows dla systemu Linux) (./windows-wsl). WSL zapewnia środowisko Linux, które płynniej współpracuje z funkcjami webforge.

---

### Brak powiadomień

webforge Desktop pokazuje powiadomienia systemowe tylko wtedy, gdy:

- powiadomienia są włączone dla webforge w ustawieniach systemu operacyjnego, oraz
...
1. Zamknij całkowicie webforge Desktop.
2. Znajdź i usuń te pliki (znajdują się w katalogu danych aplikacji webforge Desktop):

- `webforge.settings.dat` (domyślny adres URL serwera na komputerze stacjonarnym)
- `webforge.global.dat` i `webforge.workspace.*.dat` (stan interfejsu użytkownika, taki jak najnowsze serwery/projekty)
...
Jeśli masz problemy z webforge:
...
3. Upewnij się, że masz najnowszą wersję z `webforge upgrade`
...
Aby dowiedzieć się, do jakich modeli masz dostęp, uruchom `webforge models`
...
2. Jeśli problem będzie się powtarzał, spróbuj wyczyścić zapisaną konfigurację:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.local\share\webforge`
...
Jeśli napotkasz błędy wywołań API, może to wynikać z nieaktualnych pakietów dostawców. webforge dynamicznie instaluje pakiety dostawców (OpenAI, Anthropic, Google itp.) w razie potrzeby i przechowuje je lokalnie w pamięci podręcznej.
...
1. Wyczyść pamięć podręczną pakietu dostawcy:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.cache\webforge`

2. Uruchom ponownie kod webforge, aby ponownie zainstalować najnowsze pakiety dostawców

Zmusi to webforge do pobrania najnowszych wersji pakietów dostawców, co często rozwiązuje problemy ze zgodnością z parametrami modelu i zmianami API.
...
webforge wykryje, czy używasz Waylanda i wolisz `wl-clipboard`, w przeciwnym razie spróbuje znaleźć narzędzia schowka w kolejności: `xclip` i `xsel`.
