...
Narzędzia umożliwiają LLM wykonywanie działań w bazie kodu. WebForge posiada zestaw wbudowanych narzędzi, ale możesz go rozszerzyć o [narzędzia niestandardowe](/docs/custom-tools) lub [serwery MCP](/docs/mcp-servers).
...
Oto wszystkie wbudowane narzędzia dostępne w WebForge.
...
:::note
To narzędzie jest dostępne tylko wtedy, gdy `WEBFORGE_EXPERIMENTAL_LSP_TOOL=true` (lub `WEBFORGE_EXPERIMENTAL=true`).
:::
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy WebForge lub gdy zmienna środowiskowa `WEBFORGE_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).

Aby włączyć przy uruchamianiu WebForge:
