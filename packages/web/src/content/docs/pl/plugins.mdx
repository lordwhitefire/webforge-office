---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć webforge.
---

Wtyczki umożliwiające wyłączenie webforge poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania webforge.
...
- `.webforge/plugins/` - Wtyczki na poziomie projektu
- `~/.config/webforge/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/webforge/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/webforge/webforge.json`)
2. Konfiguracja projektu (`webforge.json`)
3. Globalny katalog wtyczek (`~/.config/webforge/plugins/`)
4. Katalog wtyczek projektu (`.webforge/plugins/`)
...
webforge uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z webforge do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do webforge.
...
:::note
Jeśli korzystasz z aplikacji komputerowej webforge, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj webforge czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do webforge:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod webforge. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
