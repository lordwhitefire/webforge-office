...
Umiejętności agenta pozwalają webforge odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
webforge przeszukuje te lokalizacje:

- Project config: `.webforge/skills/<name>/SKILL.md`
- Global config: `~/.config/webforge/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu webforge przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.webforge/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/webforge/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.webforge/skills/git-release/SKILL.md` w ten sposób:
...
webforge wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `webforge.json`:
...
**Dla agentów wbudowanych** (w `webforge.json`):
