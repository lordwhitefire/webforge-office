---
title: Serwer
description: Interakcja z serwerem webforge za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `webforge serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient webforge.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `webforge.local` |
...
Ustaw `WEBFORGE_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `webforge` lub ustaw `WEBFORGE_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `webforge serve`, jak i `webforge web`.
...
Kiedy uruchomisz `webforge`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera webforge do programowej interakcji z webforge.
:::

Ta architektura umożliwia obsługę wielu klientów przez webforge i programową interakcję z kodem otwartym.

Możesz uruchomić `webforge serve`, aby uruchomić samodzielny serwer. Jeśli masz
webforge TUI działa, `webforge serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki webforge [IDE](/docs/ide).
...
Serwer webforge udostępnia następujące interfejsy API.
