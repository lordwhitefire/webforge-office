...
WebForge Go to niskokosztowa subskrypcja — **5 $ za pierwszy miesiąc**, a następnie **10 $/miesiąc** — która zapewnia niezawodny dostęp do popularnych otwartych modeli do kodowania.

Go działa jak każdy inny dostawca w WebForge. Subskrybujesz WebForge Go i
...
WebForge Go daje Ci dostęp do tych modeli za **5 $ za pierwszy miesiąc**, a następnie **10 $/miesiąc**.

---

## Jak to działa

WebForge Go działa jak każdy inny dostawca w WebForge.

1. Logujesz się do **<a href={console}>WebForge Zen</a>**, subskrybujesz Go i
   kopiujesz swój klucz API.
2. Uruchamiasz komendę `/connect` w TUI, wybierasz `WebForge Go` i wklejasz
   swój klucz API.
3. Uruchom `/models` w TUI, aby zobaczyć listę modeli dostępnych w ramach Go.

:::note
Tylko jeden członek na obszar roboczy (workspace) może zasubskrybować WebForge Go.
:::
...
WebForge Go zawiera następujące limity:
...
- Kimi K2.7 Code/K2.6 — 870 tokenów wejściowych, 55 000 w pamięci podręcznej, 200 tokenów wyjściowych na żądanie
...
[ID modelu](/docs/config/#models) w Twojej konfiguracji WebForge
używa formatu `webforge-go/<model-id>`. Na przykład dla Kimi K2.7 Code należy użyć
`webforge-go/kimi-k2.7-code` w swojej konfiguracji.
...
4. Zapewnić **brak ograniczeń (no lock-in)**, pozwalając na korzystanie z dowolnego innego dostawcy wraz z WebForge.
