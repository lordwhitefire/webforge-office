...
Dzięki webforge możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie webforge używa naszego własnego motywu `webforge`.
...
webforge ma kilka wbudowanych motywów.
...
- Chcą, aby WebForge pasował do wyglądu ich terminala
...
webforge obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/webforge/themes/*.json` lub `$XDG_CONFIG_HOME/webforge/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.webforge/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.webforge/themes/*.json`
