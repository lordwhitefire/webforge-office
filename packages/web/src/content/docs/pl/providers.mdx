---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w webforge.
---

import config from "../../../../config.mjs"
export const console = config.console

webforge używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji webforge.
...
w `~/.local/share/webforge/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim webforge
config.
...
WebForge Zen to lista modeli dostarczonych przez zespół webforge, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z webforge. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od WebForge Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `WebForge Zen` i przejdź do [opencode.ai/auth](https://opencode.ai/zen).
...
Działa jak każdy inny dostawca w webforge i jest całkowicie opcjonalny w użyciu.

---

## WebForge Go

WebForge Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół webforge, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z webforge.

1. Uruchom polecenie `/connect` w TUI, wybierz `WebForge Go` i przejdź do [opencode.ai/auth](https://opencode.ai/zen).
...
Aby używać Amazon Bedrock z webforge:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania webforge:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `webforge.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w webforge nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
Możesz skonfigurować webforge do korzystania z modeli lokalnych przez [Atomic Chat](https://atomic.chat) — aplikację desktopową uruchamiającą lokalne LLM-y za serwerem API zgodnym z OpenAI (domyślny endpoint `http://127.0.0.1:1337/v1`).
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod webforge działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji webforge.
...
webforge używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować webforge
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`webforge.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Więcej informacji znajdziesz na stronie [webforge-gitlab-auth](https://www.npmjs.com/package/webforge-gitlab-auth).
...
Aby korzystać z subskrypcji GitHub Copilot z kodem webforge:
...
Aby używać Google Vertex AI z webforge:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania webforge.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą webforge, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [webforge-helicone-session](https://github.com/H2Shami/webforge-helicone-session), aby automatycznie rejestrować każdą konwersację webforge jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja webforge będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować webforge tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować webforge tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować webforge tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem webforge. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/webforge).
:::
...
Aby korzystać z Ollama Cloud z webforge:
...
3. Skopiuj klucz API do użycia w webforge.
...
6. **Ważne**: Przed użyciem modeli chmurowych w webforge musisz pobrać informacje o modelu lokalnie:
...
WebForge Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół webforge. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>WebForge Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **WebForge Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API webforge.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji webforge.
...
5. Możesz je także dostosować za pomocą konfiguracji webforge. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z webforge:
...
Możesz także dostosować modele za pomocą konfiguracji webforge. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z webforge. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `webforge.json` w katalogu projektu:
...
Pola `limit` pozwalają webforge zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `webforge auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację webforge i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji webforge.
