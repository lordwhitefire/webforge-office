...
Создайте `.webforge/commands/test.md`:
...
Вы можете добавлять собственные команды через конфигурацию webforge или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем webforge [config](/docs/config):
...
- Глобальный: `~/.config/webforge/commands/`
- Для каждого проекта: `.webforge/commands/`
...
webforge включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).
