---
title: Plugins
description: Écrivez vos propres plugins pour étendre WebForge.
---

Les plugins vous permettent d'étendre WebForge en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de WebForge.
...
- `.webforge/plugins/` - Plugins au niveau du projet
- `~/.config/webforge/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/webforge/node_modules/`.
...
1. Configuration globale (`~/.config/webforge/webforge.json`)
2. Configuration du projet (`webforge.json`)
3. Répertoire global des plugins (`~/.config/webforge/plugins/`)
4. Répertoire des plugins du projet (`.webforge/plugins/`)
...
WebForge exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK webforge pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre webforge.
...
:::note
Si vous utilisez l'application de bureau WebForge, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez webforge de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à webforge :
...
L'assistant `tool` crée un outil personnalisé que webforge peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour webforge aux côtés des outils intégrés.
