...
Avec WebForge, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, WebForge utilise notre propre thème `webforge`.
...
WebForge est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que WebForge corresponde à l'apparence de leur terminal
...
WebForge prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/webforge/themes/*.json` ou `$XDG_CONFIG_HOME/webforge/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.webforge/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.webforge/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/webforge/themes
vim ~/.config/webforge/themes/my-theme.json
```

Et pour des thèmes spécifiques au projet.

```bash no-frame
mkdir -p .webforge/themes
vim .webforge/themes/my-theme.json
```
