---
title: Serveur
description: Interagissez avec le serveur webforge via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `webforge serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client webforge peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `webforge.local` |
...
Définissez `WEBFORGE_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `webforge`, ou définissez `WEBFORGE_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `webforge serve` et à `webforge web`.
...
Lorsque vous exécutez `webforge`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur webforge pour interagir avec webforge par programmation.
:::

Cette architecture permet à webforge de prendre en charge plusieurs clients et vous permet d'interagir avec webforge par programme.

Vous pouvez exécuter `webforge serve` pour démarrer un serveur autonome. Si vous avez le
webforge TUI en cours d'exécution, `webforge serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins WebForge [IDE](/docs/ide).
...
Le serveur webforge expose les API suivantes.
