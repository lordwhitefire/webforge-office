---
title: SDK
description: Client JS de type sécurisé pour le serveur webforge.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Le SDK webforge JS/TS fournit un client de type sécurisé pour interagir avec le serveur.
Utilisez-le pour créer des intégrations et contrôler webforge par programme.
...
Créez une instance de webforge :

```javascript
import { createWebforge } from "@webforge-ai/sdk"

const { client } = await createWebforge()
```
...
Vous pouvez transmettre un objet de configuration pour personnaliser le comportement. L'instance récupère toujours votre `webforge.json`, mais vous pouvez remplacer ou ajouter une configuration en ligne :
...
console.log(`Server running at ${webforge.server.url}`)

webforge.server.close()
```

## Client uniquement

Si vous disposez déjà d'une instance en cours d'exécution de webforge, vous pouvez créer une instance client pour vous y connecter :
...
```typescript
import type { Session, Message, Part } from "@webforge-ai/sdk"
```
...
const textResults = await client.find.text({
  query: { pattern: "function.*webforge" },
})
