---
title: TUI
description: Using the WebForge terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

WebForge provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running WebForge starts the TUI for the current directory.
...
When using the WebForge TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to WebForge. Allows you to select from available providers and add their API keys.
...
Exit WebForge. _Aliases_: `/quit`, `/q`
...
This is separate from `webforge.json`, which configures server/runtime behavior.
...
- `leader_timeout` - Controls how long WebForge waits after the leader key. Defaults to `2000`.
...
Use `WEBFORGE_TUI_CONFIG` to load a custom TUI config path.
...
- `sound_pack` - Sound pack ID to use. Defaults to `webforge.default`.
