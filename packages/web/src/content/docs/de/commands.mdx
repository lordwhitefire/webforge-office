...
Erstellen Sie `.webforge/commands/test.md`:

```md title=".webforge/commands/test.md"
---
...
Sie können benutzerdefinierte Befehle über die WebForge-Konfiguration hinzufügen oder indem Sie Markdown-Dateien im Verzeichnis `commands/` erstellen.

---

### JSON

Verwenden Sie die Option `command` in Ihrem WebForge [config](/docs/config):

```json title="webforge.jsonc" {4-12}
{
...
- Global: `~/.config/webforge/commands/`
- Pro Projekt: `.webforge/commands/`

```markdown title="~/.config/webforge/commands/test.md"
---
description: Run tests with coverage
agent: build
model: anthropic/claude-3-5-sonnet-20241022
...
```md title=".webforge/commands/component.md"
---
...
```md title=".webforge/commands/create-file.md"
---
...
```md title=".webforge/commands/analyze-coverage.md"
---
...
```md title=".webforge/commands/review-changes.md"
---
...
```md title=".webforge/commands/review-component.md"
---
...
WebForge enthält mehrere integrierte Befehle wie `/init`, `/undo`, `/redo`, `/share`, `/help`; [mehr erfahren](/docs/tui#commands).
