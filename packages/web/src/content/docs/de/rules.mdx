---
title: Regeln
description: Lege eigene Anweisungen fuer webforge fest.
---

Du kannst webforge ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in webforge den Befehl `/init` aus.
...
So kann webforge sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht webforge sie zu erweitern.
...
webforge kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/webforge/AGENTS.md` speichern.
Diese gelten in allen webforge-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt WebForge dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/webforge/AGENTS.md` existiert)
...
Beim Start sucht webforge in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/webforge/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `webforge.json` oder in `~/.config/webforge/webforge.json` angeben.
...
webforge parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit webforge.json

Empfohlen ist das `instructions`-Feld in der `webforge.json`:
...
Du kannst webforge explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `webforge.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
