...
Agent Skills erlauben WebForge, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
WebForge sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft WebForge vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.webforge/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/webforge/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.webforge/skills/git-release/SKILL.md` zum Beispiel so:
...
WebForge listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `webforge.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `webforge.json`):
