...
Wenn WebForge Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\webforge\log`

Dateinamen enthalten Zeitstempel (z. B. `2025-01-09T123456.log`) und es bleiben die letzten 10 Logs erhalten.

Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `webforge --log-level DEBUG`.

---

## Speicher

webforge speichert Sitzungs- und App-Daten auf der Festplatte unter:

- **macOS/Linux**: `~/.local/share/webforge/`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\webforge`
...
WebForge Desktop startet im Hintergrund einen lokalen WebForge-Server (`webforge-cli`-Sidecar).
...
- Nur macOS: `WebForge`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
- **macOS/Linux**: `~/.config/webforge/webforge.jsonc` (oder `~/.config/webforge/webforge.json`)
- **macOS/Linux** (aeltere Installationen): `~/.local/share/webforge/webforge.jsonc`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\webforge\webforge.jsonc`
...
WebForge kann auch lokale Plugins von der Festplatte laden. Verschiebe diese voruebergehend (oder benenne den Ordner um) und starte die Desktop-App neu:

- **Globale Plugins**
  - **macOS/Linux**: `~/.config/webforge/plugins/`
  - **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\webforge\plugins`
...
Wenn das Deaktivieren von Plugins nicht hilft (oder eine Plugin-Installation haengt), leere den Cache, damit WebForge ihn neu aufbauen kann.

1. Beende WebForge Desktop komplett.
2. Loesche das Cache-Verzeichnis:

- **macOS**: Finder -> `Cmd+Shift+G` -> einfuegen: `~/.cache/webforge`
- **Linux**: loesche `~/.cache/webforge` (oder fuehre aus: `rm -rf ~/.cache/webforge`)
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.cache\webforge`

3. Starte WebForge Desktop neu.

---

### Server-Verbindungsprobleme beheben

WebForge Desktop kann entweder einen eigenen lokalen Server starten (Standard) oder sich mit einer konfigurierten Server-URL verbinden.
...
Wenn deine `webforge.json(c)` einen `server`-Abschnitt enthaelt, entferne ihn voruebergehend und starte die Desktop-App neu.

#### Umgebungsvariablen prüfen

Wenn du `WEBFORGE_PORT` in deiner Umgebung gesetzt hast, versucht die Desktop-App diesen Port fuer den lokalen Server zu nutzen.

- Setze `WEBFORGE_PORT` zurueck (oder waehle einen freien Port) und starte neu.
...
Unter Windows benoetigt WebForge Desktop die Microsoft Edge **WebView2 Runtime**. Wenn die App ein leeres Fenster zeigt oder nicht startet, installiere/aktualisiere WebView2 und versuche es erneut.

---

### Windows: Allgemeine Performance-Probleme

Wenn du langsame Performance, Dateizugriffsprobleme oder Terminal-Probleme unter Windows hast, versuche [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL bietet eine Linux-Umgebung, die nahtloser mit WebForge-Features funktioniert.

---

### Benachrichtigungen werden nicht angezeigt

WebForge Desktop zeigt Systembenachrichtigungen nur wenn:

- Benachrichtigungen fuer WebForge in den OS-Einstellungen aktiviert sind, und
...
1. Beende WebForge Desktop.
2. Finde und loesche diese Dateien (im App-Data-Verzeichnis von WebForge Desktop):

- `webforge.settings.dat` (desktop default server URL)
- `webforge.global.dat` und `webforge.workspace.*.dat` (UI state like recent servers/projects)
...
Wenn du Probleme mit WebForge hast:
...
Um zu sehen, auf welche Modelle du Zugriff hast, fuehre `webforge models` aus.
...
2. Wenn das Problem besteht, versuche deine gespeicherte Konfiguration zu loeschen:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.local\share\webforge`
...
Wenn du API-Call-Fehler erhaeltst, kann das an veralteten Provider-Paketen liegen. webforge installiert Provider-Pakete (OpenAI, Anthropic, Google, etc.) dynamisch bei Bedarf und cached sie lokal.
...
1. Leere den Provider-Paket-Cache:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.cache\webforge`

2. Starte webforge neu, um die neuesten Provider-Pakete zu installieren

Dies zwingt webforge, die neuesten Versionen der Provider-Pakete herunterzuladen, was oft Kompatibilitaetsprobleme mit Modellparametern und API-Aenderungen loest.
...
webforge erkennt, ob du Wayland nutzt und bevorzugt `wl-clipboard`, sonst versucht es `xclip` und `xsel` (in dieser Reihenfolge).
