...
Mit WebForge kannst du ein eingebautes Theme waehlen, ein an dein Terminal angepasstes Theme nutzen oder ein eigenes Theme definieren.

Standardmaessig verwendet WebForge das `webforge`-Theme.
...
WebForge liefert mehrere eingebaute Themes mit.
...
- WebForge optisch ans Terminal anpassen wollen
...
WebForge unterstuetzt ein flexibles JSON-basiertes Theme-System.
...
2. **Benutzer-Config-Verzeichnis** - Definiert in `~/.config/webforge/themes/*.json` oder `$XDG_CONFIG_HOME/webforge/themes/*.json`
3. **Projekt-Root-Verzeichnis** - Definiert in `<project-root>/.webforge/themes/*.json`
4. **Aktuelles Arbeitsverzeichnis** - Definiert in `./.webforge/themes/*.json`
