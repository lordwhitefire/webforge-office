...
WebForge Go ist ein kostengünstiges Abonnement — **5 $ für deinen ersten Monat**, danach **10 $/Monat** —, das dir zuverlässigen Zugriff auf beliebte offene Coding-Modelle bietet.

Go funktioniert wie jeder andere Provider in WebForge. Du abonnierst WebForge Go und
erhältst deinen API-Key. Es ist **völlig optional** und du musst es nicht nutzen, um
WebForge zu verwenden.
...
WebForge Go bietet dir Zugriff auf diese Modelle für **5 $ im ersten Monat**, danach **10 $/Monat**.

---

## Wie es funktioniert

WebForge Go funktioniert wie jeder andere Provider in WebForge.

1. Du meldest dich bei **<a href={console}>WebForge Zen</a>** an, abonnierst Go und kopierst deinen API-Key.
2. Du führst den Befehl `/connect` in der TUI aus, wählst `WebForge Go` und fügst deinen API-Key ein.
3. Führe `/models` in der TUI aus, um die Liste der über Go verfügbaren Modelle zu sehen.

:::note
Nur ein Mitglied pro Workspace kann WebForge Go abonnieren.
:::
...
WebForge Go beinhaltet die folgenden Limits:
...
- Kimi K2.7 Code/K2.6 — 870 Input-, 55.000 Cached-, 200 Output-Tokens pro Anfrage
...
Die [Modell-ID](/docs/config/#models) in deiner WebForge Config verwendet das Format `webforge-go/<model-id>`. Für Kimi K2.7 Code würdest du beispielsweise `webforge-go/kimi-k2.7-code` in deiner Config verwenden.
...
Wir haben WebForge Go entwickelt, um:
...
4. **Keinen Lock-in-Effekt** zu haben, indem wir dir ermöglichen, auch jeden anderen Provider mit WebForge zu nutzen.
