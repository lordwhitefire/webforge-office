---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um WebForge zu erweitern.
---

Mit Plugins können Sie WebForge erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von WebForge zu ändern.
...
- `.webforge/plugins/` – Plugins auf Projektebene
- `~/.config/webforge/plugins/` – Globale Plugins
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/webforge/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/webforge/webforge.json`)
2. Projektkonfiguration (`webforge.json`)
3. Globales Plugin-Verzeichnis (`~/.config/webforge/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.webforge/plugins/`)
...
WebForge führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein WebForge-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie WebForge erweitern können.
...
:::note
Wenn Sie die WebForge-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass WebForge `.env`-Dateien liest:
...
Plugins können WebForge auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das WebForge aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für WebForge zur Verfügung.
