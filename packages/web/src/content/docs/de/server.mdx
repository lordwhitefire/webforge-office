---
title: Server
description: Nutze den webforge-Server ueber HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Der Befehl `webforge serve` startet einen headless HTTP-Server.
Er stellt einen OpenAPI-Endpunkt bereit, den ein webforge-Client nutzen kann.
...
Setze `WEBFORGE_SERVER_PASSWORD`, um den Server mit HTTP Basic Auth zu schuetzen.
Der Benutzername ist standardmaessig `webforge`, kann aber mit `WEBFORGE_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `webforge serve` und `webforge web`.
...
Wenn du `webforge` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den webforge-Server, um webforge programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `webforge serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `webforge serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die WebForge-[IDE](/docs/ide)-Plugins.
...
Der webforge-Server stellt folgende APIs bereit.
