...
Da biste otklonili probleme s WebForge, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\webforge\log`

Datoteke evidencije se imenuju vremenskim oznakama (npr. `2025-01-09T123456.log`) i čuvaju se najnovijih 10 datoteka dnevnika.

Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `webforge --log-level DEBUG`.

---

## Pohrana

WebForge pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:

- **macOS/Linux**: `~/.local/share/webforge/`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\webforge`
...
WebForge Desktop pokreće lokalni WebForge server (`webforge-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `WebForge` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/webforge/webforge.jsonc` (ili `~/.config/webforge/webforge.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/webforge/webforge.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\webforge\webforge.jsonc`
...
WebForge također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:

- **Globalni dodaci**
  - **macOS/Linux**: `~/.config/webforge/plugins/`
  - **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\webforge\plugins`
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga WebForge mogao ponovo izgraditi.

1. Potpuno zatvorite WebForge Desktop.
2. Izbrišite keš direktorij:

- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/webforge`
- **Linux**: obrišite `~/.cache/webforge` (ili pokrenite `rm -rf ~/.cache/webforge`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\webforge`

3. Ponovo pokrenite WebForge Desktop.

---

### Rješavanje problema sa vezom na serveru

WebForge Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `webforge.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.

#### Provjerite varijable okruženja

Ako ste postavili `WEBFORGE_PORT` u svom okruženju, desktop aplikacija će pokušati da koristi taj port za lokalni server.

- Poništite `WEBFORGE_PORT` (ili odaberite slobodan port) i ponovo pokrenite.
...
Na Windows-u, WebForge Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa WebForge karakteristikama.

---

### Obavještenja se ne prikazuju

WebForge Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za WebForge u postavkama vašeg OS-a, i
...
1. Zatvorite WebForge Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka WebForge Desktop aplikacije):

- `webforge.settings.dat` (URL zadanog servera za desktop)
- `webforge.global.dat` i `webforge.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
Da saznate kojim modelima imate pristup, pokrenite `webforge models`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.local\share\webforge`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. WebForge dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\webforge`

2. Ponovo pokrenite WebForge da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti WebForge da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
WebForge će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
