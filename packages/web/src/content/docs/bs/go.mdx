...
WebForge Go je povoljna pretplata — **$5 za vaš prvi mjesec**, a zatim **$10/mjesečno** — koja vam pruža pouzdan pristup popularnim otvorenim modelima za programiranje.

Go radi kao bilo koji drugi provajder u WebForge-u. Pretplatite se na WebForge Go i
dobijete svoj API ključ. On je **potpuno opcionalan** i ne morate ga koristiti da
biste koristili WebForge.
...
:::tip
Testirali smo odabranu grupu modela i provajdera koji dobro rade sa WebForge-om.
:::
...
WebForge Go vam daje pristup ovim modelima za **$5 za vaš prvi mjesec**, a zatim **$10/mjesečno**.

---

## Kako funkcioniše

WebForge Go radi kao bilo koji drugi provajder u WebForge-u.

1. Prijavite se na **<a href={console}>WebForge Zen</a>**, pretplatite se na Go i
   kopirajte svoj API ključ.
2. Pokrenite komandu `/connect` u TUI-ju, odaberite `WebForge Go` i zalijepite
   svoj API ključ.
3. Pokrenite `/models` u TUI-ju da vidite listu modela dostupnih kroz Go.

:::note
Samo jedan član po radnom prostoru (workspace) može se pretplatiti na WebForge Go.
:::
...
WebForge Go uključuje sljedeća ograničenja:
...
- Kimi K2.7 Code/K2.6 — 870 ulaznih, 55,000 keširanih, 200 izlaznih tokena po zahtjevu
...
[Model id](/docs/config/#models) u vašoj WebForge konfiguraciji
koristi format `webforge-go/<model-id>`. Na primjer, za Kimi K2.7 Code, koristili biste
`webforge-go/kimi-k2.7-code` u svojoj konfiguraciji.
...
Napravili smo WebForge Go da bismo:
...
4. Osigurali da **nema zaključavanja (no lock-in)**, omogućavajući vam da uz WebForge koristite i bilo kojeg drugog provajdera.
