---
title: LSP serveri
description: WebForge se integriše sa vašim LSP serverima.
---

WebForge se može integrisati sa Language Server Protocol (LSP) serverima kako bi koristio dijagnostiku kao feedback za agenta.

## Ugrađeni

WebForge dolazi sa nekoliko ugrađenih LSP servera za popularne jezike:
...
:::note
Možete onemogućiti automatska preuzimanja LSP servera tako što ćete postaviti varijablu okruženja `WEBFORGE_DISABLE_LSP_DOWNLOAD` na `true`.
:::
...
Kada je LSP omogućen i webforge otvori fajl, on:
...
Možete omogućiti i prilagoditi LSP servere kroz `lsp` odjeljak u vašoj webforge konfiguraciji.
