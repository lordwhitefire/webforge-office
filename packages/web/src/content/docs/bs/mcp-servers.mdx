...
Možete dodati vanjske alate u WebForge koristeći _Model Context Protocol_, ili MCP. WebForge podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [WebForge Config](https://opencode.ai/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/webforge`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
WebForge automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, WebForge će:
...
Ako server zahtijeva autentifikaciju, WebForge će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `webforge mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, WebForge sigurno čuva tokene u `~/.local/share/webforge/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u WebForge, zajedno s ugrađenim alatima. Možete njima upravljati kroz WebForge konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete WebForge sa vašim Sentry nalogom.
