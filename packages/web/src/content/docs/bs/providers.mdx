---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u WebForge.
---

import config from "../../../../config.mjs"
export const console = config.console

WebForge koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj WebForge konfiguraciji.
...
u `~/.local/share/webforge/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem WebForge
config.
...
WebForge Zen je lista modela koje je obezbedio WebForge tim koji su bili
testirano i potvrđeno da dobro radi sa WebForge. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa WebForge Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite webforge i idite na [opencode.ai/auth](https://opencode.ai/auth).
...
Radi kao i svaki drugi provajder u WebForge i potpuno je opcionalan za korištenje.

---

## WebForge Go

WebForge Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža WebForge tim i koji su testirani i verificirani da dobro rade s WebForge-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `WebForge Go` i idite na [opencode.ai/auth](https://opencode.ai/zen).
...
Da biste koristili Amazon Bedrock s WebForge:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete webforge:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `webforge.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u WebForge.
:::
...
Možete konfigurirati webforge za korištenje lokalnih modela preko [Atomic Chata](https://atomic.chat) — desktop aplikacije koja pokreće lokalne LLM-ove iza OpenAI-kompatibilnog API servera (zadana krajnja tačka `http://127.0.0.1:1337/v1`).
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi webforge ispravno radio.
   :::
...
Također možete dodati modele kroz svoju webforge konfiguraciju.
...
da pohrani token u webforge auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
WebForge koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate WebForge
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`webforge.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Više dokumentacije na početnoj stranici [webforge-gitlab-auth](https://www.npmjs.com/package/webforge-gitlab-auth).

##### Konfiguracija

Prilagodite putem `webforge.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s webforge:
...
Za korištenje Google Vertex AI s WebForge:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće webforge.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem webforgea, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [webforge-helicone-session](https://github.com/H2Shami/webforge-helicone-session) da automatski prijavite svaki WebForge razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki WebForge razgovor naveden kao zasebna sesija.
...
Možete konfigurirati webforge za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati webforge za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati webforge za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za WebForge. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/webforge) za detalje.
:::
...
Da biste koristili Ollama Cloud s WebForge:
...
3. Kopirajte API ključ za korištenje u WebForge.
...
6. **Važno**: Prije upotrebe modela oblaka u WebForge, morate lokalno povući informacije o modelu:
...
WebForge Zen je lista testiranih i verifikovanih modela koju je obezbedio WebForge tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>WebForge Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **WebForge Zen**.

```txt
   /connect
```

3. Unesite svoj WebForge API ključ.
...
Također možete dodati dodatne modele putem vaše webforge konfiguracije.
...
5. Također ih možete prilagoditi putem vaše webforge konfiguracije. Evo primjera navođenja provajdera
...
Da biste koristili [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) sa Webforgeom:
...
Također možete prilagoditi modele kroz svoju webforge konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s webforge-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `webforge.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju WebForge da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `webforge auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite WebForge konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj webforge konfiguraciji.
