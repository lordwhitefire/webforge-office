---
title: Konfiguracija
description: Korištenje WebForge JSON konfiguracije.
---

Možete konfigurirati WebForge koristeći JSON konfiguracijski fajl.

---

## Format

WebForge podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
1. **Udaljena konfiguracija** (od `.well-known/webforge`) - organizacijske postavke
2. **Globalna konfiguracija** (`~/.config/webforge/webforge.json`) - korisničke preferencije
3. **Prilagođena konfiguracija** (`WEBFORGE_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`webforge.json` u projektu) - postavke specifične za projekat
5. **`.webforge` direktoriji** - agenti, komande, dodaci
6. **Inline konfiguracija** (`WEBFORGE_CONFIG_CONTENT` env var) - runtime preinačenja
...
:::note
Direktoriji `.webforge` i `~/.config/webforge` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
:::
...
Organizacije mogu pružiti zadanu konfiguraciju preko `.well-known/webforge` krajnje tačke. Ovo se automatski preuzima kada se autentifikujete kod provajdera koji to podržava.
...
Postavite svoju globalnu WebForge konfiguraciju u `~/.config/webforge/webforge.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.

Za postavke specifične za TUI, koristite `~/.config/webforge/tui.json`.
...
Dodajte `webforge.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
...
Kada se WebForge pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
Navedite prilagođenu putanju konfiguracijske datoteke koristeći varijablu okruženja `WEBFORGE_CONFIG`.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `WEBFORGE_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.webforge` direktorij, i trebali bi pratiti istu strukturu.
...
Prilagođeni direktorij se učitava nakon direktorija globalne konfiguracije i `.webforge`, tako da **može nadjačati** njihove postavke.
...
Koristite `WEBFORGE_TUI_CONFIG` da pokažete na prilagođeni TUI konfiguracijski fajl.

Stari `theme`, `keybinds`, i `tui` ključevi u `webforge.json` su zastarjeli i automatski će se migrirati kada je to moguće.
...
Možete konfigurirati postavke servera za naredbe `webforge serve` i `webforge web` putem opcije `server`.
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš WebForge server.
- `mdnsDomain` - Prilagođeno ime domene za mDNS servis. Zadano je `webforge.local`. Korisno za pokretanje više instanci na istoj mreži.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj WebForge konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, WebForge pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/webforge/agents/` ili `.webforge/agents/`. [Saznajte više ovdje](/docs/agents).
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, WebForge će se vratiti na `"build"` s upozorenjem.

Ova postavka se primjenjuje na sva sučelja: TUI, CLI (`webforge run`), desktop aplikaciju i GitHub Action.
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/webforge/commands/` ili `.webforge/commands/`. [Saznajte više ovdje](/docs/commands).
...
WebForge će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, WebForge **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju WebForge sa prilagođenim alatima, kukicama i integracijama.

Postavite datoteke dodataka u `.webforge/plugins/` ili `~/.config/webforge/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
...
Ovo je korisno kada želite da ograničite WebForge da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.
