...
Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. WebForge dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u WebForge.
...
:::note
Ovaj alat je dostupan samo kada je `WEBFORGE_EXPERIMENTAL_LSP_TOOL=true` (ili `WEBFORGE_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz WebForge provajdera ili kada je varijabla `WEBFORGE_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju WebForge:
