...
U WebForge mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, WebForge koristi nasu `webforge` temu.
...
WebForge dolazi sa vise ugradenih tema.
...
- Zele da WebForge odgovara izgledu njihovog terminala
...
WebForge podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/webforge/themes/*.json` ili `$XDG_CONFIG_HOME/webforge/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.webforge/themes/*.json`
4. **Trenutni radni direktorij** - `./.webforge/themes/*.json`
