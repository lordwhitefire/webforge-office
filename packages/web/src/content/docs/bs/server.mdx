---
title: Server
description: Komunicirajte s webforge serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `webforge serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji webforge klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `webforge.local` |
...
Postavite `WEBFORGE_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `webforge`, ili postavite `WEBFORGE_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `webforge serve` i za `webforge web`.
...
Kada pokrenete `webforge`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite webforge server za programsku interakciju sa webforge-om.
:::

Ova arhitektura omogucava webforge podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `webforge serve` da startate standalone server. Ako je webforge TUI vec pokrenut, `webforge serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste WebForge [IDE](/docs/ide) pluginovi.
...
webforge server izlaže sljedece API-je.
