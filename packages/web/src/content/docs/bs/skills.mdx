...
Agent skills omogucavaju WebForge da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
WebForge pretrazuje ove lokacije:

- Konfiguracija projekta: `.webforge/skills/<name>/SKILL.md`
- Globalna konfiguracija: `~/.config/webforge/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, WebForge ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
Usput ucitava sve odgovarajuce `skills/*/SKILL.md` u `.webforge/` i odgovarajuce `.claude/skills/*/SKILL.md` ili `.agents/skills/*/SKILL.md`.

Globalne definicije se takoder ucitavaju iz `~/.config/webforge/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Kreirajte `.webforge/skills/git-release/SKILL.md` ovako:
...
WebForge navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `webforge.json`:
...
**Za ugradene agente** (u `webforge.json`):
