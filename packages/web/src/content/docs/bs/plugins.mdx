---
title: Dodaci
description: Napišite vlastite dodatke za proširenje WebForge.
---

Dodaci vam omogućavaju da proširite WebForge spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja WebForge.
...
- `.webforge/plugins/` - Dodaci na nivou projekta
- `~/.config/webforge/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/webforge/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/webforge/webforge.json`)
2. Konfiguracija projekta (`webforge.json`)
3. Globalni direktorij dodataka (`~/.config/webforge/plugins/`)
4. Direktorij dodataka projekta (`.webforge/plugins/`)
...
WebForge pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
- `client`: Webforge SDK klijent za interakciju sa AI.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje WebForge.
...
Spriječite webforge da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u webforge:
...
Pomoćnik `tool` kreira prilagođeni alat koji webforge može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za webforge zajedno sa ugrađenim alatima.
