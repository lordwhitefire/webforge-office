...
With WebForge you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, WebForge uses our own `webforge` theme.
...
- Want WebForge to match their terminal's appearance
...
WebForge supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/webforge/themes/*.json` or `$XDG_CONFIG_HOME/webforge/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.webforge/themes/*.json`
4. **Current working directory** - Defined in `./.webforge/themes/*.json`
