---
title: Server
description: Interact with webforge server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `webforge serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an webforge client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `webforge.local` |
...
Set `WEBFORGE_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `webforge`, or set `WEBFORGE_SERVER_USERNAME` to override it. This applies to both `webforge serve` and `webforge web`.
...
When you run `webforge` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the webforge server to interact with webforge programmatically.
:::

This architecture lets webforge support multiple clients and allows you to interact with webforge programmatically.

You can run `webforge serve` to start a standalone server. If you have the
webforge TUI running, `webforge serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the WebForge [IDE](/docs/ide) plugins.
...
The webforge server exposes the following APIs.
