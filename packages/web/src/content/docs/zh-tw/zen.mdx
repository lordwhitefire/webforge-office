---
title: Zen
description: 由 WebForge 提供的精選模型列表。
---
...
WebForge Zen 是由 WebForge 團隊提供、經過測試與驗證的模型清單。

Zen 的運作方式和 WebForge 中的其他供應商一樣。你登入 WebForge Zen 並取得
...
:::tip
我們測試了一組與 WebForge 搭配良好的精選模型與供應商。
:::
...
WebForge Zen 是一個 AI 閘道，讓你可以存取這些模型。

---

## 運作方式

WebForge Zen 的運作方式和 WebForge 中的其他供應商一樣。

1. 你登入 **<a href={console}>WebForge Zen</a>**，加入帳單資訊，然後複製你的 API 金鑰。
2. 你在 TUI 中執行 `/connect` 指令，選擇 WebForge Zen，然後貼上你的 API 金鑰。
...
WebForge 設定中的 [模型 ID](/docs/config/#models) 會使用 `webforge/<model-id>`
格式。例如，如果是 GPT 5.5，你會在設定中使用 `webforge/gpt-5.5`。
...
- DeepSeek V4 Flash Free 在 WebForge 上限時提供。團隊正在利用這段時間收集回饋並改進模型。
- MiMo-V2.5 Free 在 WebForge 上限時提供。團隊正在利用這段時間收集回饋並改進模型。
- North Mini Code Free 在 WebForge 上限時提供。團隊正在利用這段時間收集回饋並改進模型。
- Nemotron 3 Ultra Free 在 WebForge 上限時提供。團隊正在利用這段時間收集回饋並改進模型。
- Big Pickle 是一個隱身模型，在 WebForge 上限時免費提供。團隊正在利用這段時間收集回饋並改進模型。
