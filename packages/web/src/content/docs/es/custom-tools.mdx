...
description: Cree herramientas que LLM pueda llamar en webforge.
---

Las herramientas personalizadas son funciones que usted crea y que el LLM puede llamar durante las conversaciones. Trabajan junto con las [herramientas integradas](/docs/tools) de webforge como `read`, `write` y `bash`.
...
- Localmente colocándolos en el directorio `.webforge/tools/` de tu proyecto.
- O de forma global, colocándolos en `~/.config/webforge/tools/`.
...
```ts title=".webforge/tools/database.ts" {1}
...
```ts title=".webforge/tools/math.ts"
import { tool } from "@webforge-ai/plugin"
...
```ts title=".webforge/tools/bash.ts"
import { tool } from "@webforge-ai/plugin"
...
```ts title=".webforge/tools/project.ts" {8}
import { tool } from "@webforge-ai/plugin"
...
```python title=".webforge/tools/add.py"
import sys
...
```ts title=".webforge/tools/python-add.ts" {10}
import { tool } from "@webforge-ai/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".webforge/tools/add.py")
