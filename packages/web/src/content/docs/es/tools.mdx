...
Las herramientas permiten que LLM realice acciones en su código base. WebForge viene con un conjunto de herramientas integradas, pero puede ampliarlo con [herramientas personalizadas](/docs/custom-tools) o [servidores MCP](/docs/mcp-servers).
...
Aquí están todas las herramientas integradas disponibles en WebForge.
...
:::note
Esta herramienta solo está disponible cuando `WEBFORGE_EXPERIMENTAL_LSP_TOOL=true` (o `WEBFORGE_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor WebForge o cuando la variable de entorno `WEBFORGE_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar WebForge:

```bash
WEBFORGE_ENABLE_EXA=1 webforge
```
