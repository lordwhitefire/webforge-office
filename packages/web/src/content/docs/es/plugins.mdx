---
title: Complementos
description: Escriba sus propios complementos para extender WebForge.
---

Los complementos le permiten extender WebForge al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de WebForge.
...
- `.webforge/plugins/` - Complementos a nivel de proyecto
- `~/.config/webforge/plugins/` - Complementos globales
...
```json title="webforge.json"
{
  ...
  "plugin": ["webforge-helicone-session", "webforge-wakatime", "@my-org/custom-plugin"]
}
```
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/webforge/node_modules/`.
...
1. Configuración global (`~/.config/webforge/webforge.json`)
2. Configuración del proyecto (`webforge.json`)
3. Directorio global de complementos (`~/.config/webforge/plugins/`)
4. Directorio de complementos del proyecto (`.webforge/plugins/`)
...
```json title=".webforge/package.json"
{
  "dependencies": {
    "shescape": "^2.1.0"
  }
...
WebForge ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.

```ts title=".webforge/plugins/my-plugin.ts"
...
```js title=".webforge/plugins/example.js"
...
- `client`: Un cliente SDK webforge para interactuar con la IA.
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar webforge.
...
```js title=".webforge/plugins/notification.js"
export const NotificationPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
    event: async ({ event }) => {
      // Send notification on session completion
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "webforge"'`
      }
    },
  }
}
...
:::note
Si está utilizando la aplicación de escritorio WebForge, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que webforge lea archivos `.env`:

```javascript title=".webforge/plugins/env-protection.js"
...
```javascript title=".webforge/plugins/inject-env.js"
...
Los complementos también pueden agregar herramientas personalizadas a webforge:

```ts title=".webforge/plugins/custom-tools.ts"
import { type Plugin, tool } from "@webforge-ai/plugin"
...
El ayudante `tool` crea una herramienta personalizada a la que webforge puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para webforge junto con las herramientas integradas.
...
```ts title=".webforge/plugins/compaction.ts"
import type { Plugin } from "@webforge-ai/plugin"
...
```ts title=".webforge/plugins/custom-compaction.ts"
import type { Plugin } from "@webforge-ai/plugin"
