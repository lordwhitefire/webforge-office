...
Las habilidades del agente permiten a WebForge descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
WebForge busca estas ubicaciones:

- Configuración del proyecto: `.webforge/skills/<name>/SKILL.md`
- Configuración global: `~/.config/webforge/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, WebForge sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.webforge/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.config/webforge/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.webforge/skills/git-release/SKILL.md` así:
...
WebForge enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `webforge.json`:
...
**Para agentes integrados** (en `webforge.json`):
