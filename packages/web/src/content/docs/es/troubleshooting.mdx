...
Para depurar problemas con WebForge, comience verificando los registros y los datos locales que almacena en el disco.
...
- **macOS/Linux**: `~/.local/share/webforge/log/`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\webforge\log`
...
Puede configurar el nivel de registro con la opción de línea de comandos `--log-level` para obtener información de depuración más detallada. Por ejemplo, `webforge --log-level DEBUG`.

---

## Almacenamiento

webforge almacena datos de sesión y otros datos de aplicaciones en el disco en:

- **macOS/Linux**: `~/.local/share/webforge/`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\webforge`
...
WebForge Desktop ejecuta un servidor WebForge local (el sidecar `webforge-cli`) en segundo plano. La mayoría de los problemas se deben a un complemento que no funciona correctamente, una memoria caché dañada o una mala configuración del servidor.
...
- Solo macOS: menú `WebForge` -> **Recargar vista web** (ayuda si la interfaz de usuario está en blanco/congelada).
...
- **macOS/Linux**: `~/.config/webforge/webforge.jsonc` (o `~/.config/webforge/webforge.json`)
- **macOS/Linux** (instalaciones anteriores): `~/.local/share/webforge/webforge.jsonc`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\webforge\webforge.jsonc`
...
WebForge también puede cargar complementos locales desde el disco. Quítelos temporalmente del camino (o cambie el nombre de la carpeta) y reinicie la aplicación de escritorio:

- **Complementos globales**
  - **macOS/Linux**: `~/.config/webforge/plugins/`
  - **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\webforge\plugins`
- **Complementos de proyecto** (solo si usas la configuración por proyecto)
  - `<your-project>/.webforge/plugins/`
...
Si deshabilitar los complementos no ayuda (o la instalación de un complemento está bloqueada), borre el caché para que WebForge pueda reconstruirlo.

1. Salga de WebForge Desktop por completo.
2. Elimine el directorio de caché:

- **macOS**: Buscador -> `Cmd+Shift+G` -> pegar `~/.cache/webforge`
- **Linux**: eliminar `~/.cache/webforge` (o ejecutar `rm -rf ~/.cache/webforge`)
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.cache\webforge`

3. Reinicie el escritorio WebForge.

---

### Solucionar problemas de conexión del servidor

WebForge Desktop puede iniciar su propio servidor local (predeterminado) o conectarse a la URL de un servidor que haya configurado.
...
Si su `webforge.json(c)` contiene una sección `server`, elimínela temporalmente y reinicie la aplicación de escritorio.

#### Verificar variables de entorno

Si tiene `WEBFORGE_PORT` configurado en su entorno, la aplicación de escritorio intentará usar ese puerto para el servidor local.

- Desarme `WEBFORGE_PORT` (o elija un puerto libre) y reinicie.
...
En Windows, el escritorio WebForge requiere Microsoft Edge **WebView2 Runtime**. Si la aplicación se abre en una ventana en blanco o no se inicia, instale/actualice WebView2 e inténtelo nuevamente.

---

### Windows: Problemas generales de rendimiento

Si tiene un rendimiento lento, problemas de acceso a archivos o problemas de terminal en Windows, intente usar [WSL (Windows Subsistema para Linux)](/docs/windows-wsl). WSL proporciona un entorno Linux que funciona de manera más fluida con las funciones de WebForge.

---

### Notificaciones que no se muestran

WebForge Desktop solo muestra notificaciones del sistema cuando:

- las notificaciones están habilitadas para WebForge en la configuración de su sistema operativo, y
...
1. Salga del escritorio WebForge.
2. Busque y elimine estos archivos (se encuentran en el directorio de datos de la aplicación de escritorio WebForge):

- `webforge.settings.dat` (URL del servidor predeterminado de escritorio)
- `webforge.global.dat` y `webforge.workspace.*.dat` (estado de la interfaz de usuario como servidores/proyectos recientes)
...
Si tiene problemas con WebForge:
...
1. **Informar problemas el GitHub**
   ...
   [**github.com/anomalyco/webforge/issues**](https://github.com/anomalyco/webforge/issues)
...
3. Asegúrese de tener la última versión con `webforge upgrade`
...
Para saber a qué modelos tiene acceso, ejecute `webforge models`
...
2. Si el problema persiste, intente borrar la configuración almacenada:
   ...
   ```bash
   rm -rf ~/.local/share/webforge
   ```

   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.local\share\webforge`
...
Si encuentra errores de llamada API, esto puede deberse a paquetes de proveedores obsoletos. webforge instala dinámicamente paquetes de proveedores (OpenAI, Anthropic, Google, etc.) según sea necesario y los almacena en caché localmente.
...
1. Borre la caché del paquete del proveedor:
   ...
   ```bash
   rm -rf ~/.cache/webforge
   ```

   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.cache\webforge`

2. Reinicie webforge para reinstalar los paquetes del proveedor más recientes.

Esto obligará a webforge a descargar las versiones más recientes de los paquetes del proveedor, lo que a menudo resuelve problemas de compatibilidad con los parámetros del modelo y los cambios de API.
...
webforge detectará si estás usando Wayland y prefieres `wl-clipboard`; de lo contrario, intentará encontrar herramientas del portapapeles en el orden de: `xclip` y `xsel`.
