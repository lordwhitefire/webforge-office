...
WebForge Go es una suscripción de bajo costo — **$5 por tu primer mes**, luego **$10/mes** — que te brinda acceso confiable a modelos abiertos de programación populares.

Go funciona como cualquier otro proveedor en WebForge. Te suscribes a WebForge Go y
...
WebForge Go te da acceso a estos modelos por **$5 por tu primer mes**, luego **$10/mes**.

---

## Cómo funciona

WebForge Go funciona como cualquier otro proveedor en WebForge.

1. Inicias sesión en **<a href={console}>WebForge Zen</a>**, te suscribes a Go y
   copias tu API key.
2. Ejecutas el comando `/connect` en la TUI, seleccionas `WebForge Go` y pegas
   tu API key.
3. Ejecutas `/models` en la TUI para ver la lista de modelos disponibles a través de Go.

:::note
Solo un miembro por espacio de trabajo puede suscribirse a WebForge Go.
:::
...
WebForge Go incluye los siguientes límites:
...
- Kimi K2.7 Code/K2.6 — 870 tokens de entrada, 55,000 en caché, 200 tokens de salida por petición
...
El [ID del modelo](/docs/config/#models) en tu configuración de WebForge
usa el formato `webforge-go/<model-id>`. Por ejemplo, para Kimi K2.7 Code, usarías
`webforge-go/kimi-k2.7-code` en tu configuración.
...
4. Evitar la **dependencia (lock-in)** al permitirte usar también cualquier otro proveedor con WebForge.
