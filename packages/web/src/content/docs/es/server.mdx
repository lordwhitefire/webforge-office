---
title: Servidor
description: Interactuar con el servidor webforge a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `webforge serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente webforge puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `webforge.local` |
...
Configure `WEBFORGE_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `webforge`, o configure `WEBFORGE_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `webforge serve` como a `webforge web`.
...
Cuando ejecuta `webforge`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor webforge para interactuar con webforge mediante programación.
:::

Esta arquitectura permite que webforge admita múltiples clientes y le permite interactuar con webforge mediante programación.

Puede ejecutar `webforge serve` para iniciar un servidor independiente. Si tienes el
webforge TUI ejecutándose, `webforge serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos WebForge [IDE](/docs/ide).
...
El servidor webforge expone las siguientes API.
