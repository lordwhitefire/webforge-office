...
Con WebForge puede seleccionar uno de varios temas integrados, usar un tema que se adapte al tema de su terminal o definir su propio tema personalizado.

De forma predeterminada, WebForge usa nuestro propio tema `webforge`.
...
WebForge viene con varios temas integrados.
...
- Quiere que WebForge coincida con la apariencia de su terminal
...
WebForge admite un sistema de temas flexible basado en JSON que permite a los usuarios crear y personalizar temas fácilmente.
...
2. **Directorio de configuración de usuario**: definido en `~/.config/webforge/themes/*.json` o `$XDG_CONFIG_HOME/webforge/themes/*.json`
3. **Directorio raíz del proyecto**: definido en `<project-root>/.webforge/themes/*.json`
4. **Directorio de trabajo actual** - Definido en `./.webforge/themes/*.json`
