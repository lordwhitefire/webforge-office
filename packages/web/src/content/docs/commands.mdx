...
Create `.webforge/commands/test.md`:
...
You can add custom commands through the WebForge config or by creating markdown files in the `commands/` directory.

---

### JSON

Use the `command` option in your WebForge [config](/docs/config):
...
webforge includes several built-in commands like `/init`, `/undo`, `/redo`, `/share`, `/help`; [learn more](/docs/tui#commands).
