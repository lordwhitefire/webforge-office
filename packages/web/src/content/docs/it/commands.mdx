...
Crea `.webforge/commands/test.md`:
...
Puoi aggiungere comandi personalizzati tramite la config di WebForge o creando file markdown nella directory `commands/`.

---

### JSON

Usa l'opzione `command` nella [config](/docs/config) di WebForge:
...
- Globale: `~/.config/webforge/commands/`
- Per progetto: `.webforge/commands/`
...
webforge include diversi comandi integrati come `/init`, `/undo`, `/redo`, `/share`, `/help`; [scopri di piu](/docs/tui#commands).
