...
Puoi aggiungere strumenti esterni a WebForge usando il _Model Context Protocol_, o MCP. WebForge supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [WebForge Config](https://opencode.ai/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
Le organizzazioni possono fornire server MCP predefiniti tramite l'endpoint `.well-known/webforge`. Questi server possono essere disabilitati di default, lasciando agli utenti la possibilita' di fare opt-in su quelli necessari.
...
WebForge gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, WebForge:
...
Se il server richiede autenticazione, WebForge ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `webforge mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, WebForge memorizzera' i token in modo sicuro in `~/.local/share/webforge/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in WebForge insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione WebForge come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare WebForge al tuo account Sentry.
