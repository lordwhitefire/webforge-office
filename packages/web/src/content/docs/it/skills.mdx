...
Le skill degli agenti permettono a WebForge di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
WebForge cerca in queste posizioni:

- Config di progetto: `.webforge/skills/<name>/SKILL.md`
- Config globale: `~/.config/webforge/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, WebForge risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.webforge/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/webforge/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.webforge/skills/git-release/SKILL.md` cosi':
...
WebForge elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `webforge.json`:
...
**Per agenti integrati** (in `webforge.json`):
