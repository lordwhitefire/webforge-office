---
title: Server LSP
description: WebForge si integra con i tuoi server LSP.
---

WebForge può integrarsi con server Language Server Protocol (LSP) per usare la diagnostica come feedback per l'agente.

---

## Integrati

WebForge include diversi server LSP integrati per linguaggi popolari:
...
:::note
Puoi disabilitare il download automatico dei server LSP impostando la variabile d'ambiente `WEBFORGE_DISABLE_LSP_DOWNLOAD` a `true`.
:::
...
Quando LSP è abilitato e webforge apre un file:
...
Puoi abilitare e personalizzare i server LSP tramite la sezione `lsp` nella configurazione di webforge.
