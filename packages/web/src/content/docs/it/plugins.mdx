---
title: Plugin
description: Scrivi plugin per estendere WebForge.
---

I plugin ti permettono di estendere WebForge agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di WebForge.
...
- `.webforge/plugins/` - plugin a livello progetto
- `~/.config/webforge/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/webforge/node_modules/`.
...
1. Config globale (`~/.config/webforge/webforge.json`)
2. Config di progetto (`webforge.json`)
3. Directory plugin globale (`~/.config/webforge/plugins/`)
4. Directory plugin di progetto (`.webforge/plugins/`)
...
WebForge esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di webforge per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere webforge.
...
:::note
Se usi l'app desktop di WebForge, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a webforge di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a webforge:
...
L'helper `tool` crea uno strumento personalizzato che webforge può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in webforge insieme agli strumenti integrati.
