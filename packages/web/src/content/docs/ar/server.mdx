---
title: الخادم
description: تفاعل مع خادم webforge عبر HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

يشغّل الأمر `webforge serve` خادما HTTP دون واجهة ويعرض نقطة نهاية OpenAPI يمكن لعميل webforge استخدامها.
...
| `--mdns-domain` | اسم نطاق مخصص لخدمة mDNS              | `webforge.local` |
...
عيّن `WEBFORGE_SERVER_PASSWORD` لحماية الخادم باستخدام مصادقة HTTP الأساسية. اسم المستخدم افتراضيا هو `webforge`، أو عيّن `WEBFORGE_SERVER_USERNAME` لتغييره. ينطبق ذلك على كل من `webforge serve` و `webforge web`.
...
عند تشغيل `webforge` يبدأ تشغيل واجهة terminal تفاعلية (TUI) وخادما. تكون الـ TUI هي
العميل الذي يتحدث إلى الخادم. يوفّر الخادم نقطة نهاية لمواصفة OpenAPI 3.1.
وتُستخدم هذه النقطة أيضا لتوليد [SDK](/docs/sdk).

:::tip
استخدم خادم webforge للتفاعل مع webforge برمجيا.
:::

تتيح هذه البنية لـ webforge دعم عدة عملاء وتمكّنك من التفاعل مع webforge برمجيا.

يمكنك تشغيل `webforge serve` لبدء خادم مستقل. إذا كانت واجهة webforge في terminal (TUI)
قيد التشغيل، فسيبدأ `webforge serve` خادما جديدا.
...
يمكن استخدام نقطة النهاية [`/tui`](#tui) للتحكم في الـ TUI عبر الخادم. على سبيل المثال، يمكنك تعبئة الموجّه مسبقا أو تشغيله. يُستخدم هذا الإعداد بواسطة ملحقات WebForge لـ [IDE](/docs/ide).
...
يعرض خادم webforge واجهات API التالية.
