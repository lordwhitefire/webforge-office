---
title: المزوّدون
description: استخدام أي موفّر LLM في WebForge.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم WebForge ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات WebForge.
...
في `~/.local/share/webforge/auth.json`.

---

### الإعدادات

يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات WebForge.
...
WebForge Zen هي قائمة نماذج يوفّرها فريق WebForge وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع WebForge. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع WebForge Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر webforge، ثم انتقل إلى [opencode.ai/auth](https://opencode.ai/auth).
...
يعمل مثل أي مزوّد آخر في WebForge واستخدامه اختياري بالكامل.

---

## WebForge Go

WebForge Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق WebForge، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع WebForge.

1. شغّل الأمر `/connect` في TUI، واختر `WebForge Go`، ثم انتقل إلى [opencode.ai/auth](https://opencode.ai/zen).
...
لاستخدام Amazon Bedrock مع WebForge:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل webforge:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `webforge.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل WebForge غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
يمكنك تكوين webforge لاستخدام النماذج المحلية عبر [Atomic Chat](https://atomic.chat)، وهو تطبيق سطح مكتب يقوم بتشغيل LLMs المحلية خلف خادم API متوافق مع OpenAI (نقطة النهاية الافتراضية `http://127.0.0.1:1337/v1`).
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل webforge بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات webforge.
...
تخزين الرمز في مساحة تخزين مصادقة webforge.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم WebForge نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر WebForge
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`webforge.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
مزيد من التوثيق على صفحة [webforge-gitlab-auth](https://www.npmjs.com/package/webforge-gitlab-auth).

##### التهيئة

خصّص عبر `webforge.json`:
...
لاستخدام اشتراك GitHub Copilot مع webforge:
...
لاستخدام Google Vertex AI مع WebForge:
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر webforge، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [webforge-helicone-session](https://github.com/H2Shami/webforge-helicone-session) لتسجيل كل محادثة WebForge تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة WebForge مدرجة كجلسة مستقلة.
...
يمكنك ضبط webforge لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط webforge لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط webforge لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع WebForge. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/webforge) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع WebForge:
...
3. انسخ مفتاح API لاستخدامه في WebForge.
...
6. **مهم**: قبل استخدام نماذج السحابة في WebForge، يجب سحب معلومات النموذج محليا:
...
WebForge Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق WebForge. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>WebForge Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **WebForge Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ WebForge.
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات webforge.
...
5. يمكنك أيضا تخصيصها عبر إعدادات webforge. إليك مثالا على تحديد مزوّد
...
لاستخدام [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) مع Webforge:
...
يمكنك أيضا تخصيص النماذج عبر إعدادات webforge. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع webforge. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
4. أنشئ أو حدّث ملف `webforge.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ WebForge بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `webforge auth list` لمعرفة ما إذا كانت بيانات الاعتماد
   الخاصة بالمزوّد قد أضيفت إلى إعداداتك.

   لا ينطبق هذا على مزوّدات مثل Amazon Bedrock التي تعتمد على متغيرات البيئة لـ auth.

2. للمزوّدات المخصّصة، راجع إعدادات webforge وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات webforge.
