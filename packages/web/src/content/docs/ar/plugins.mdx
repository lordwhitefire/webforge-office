---
title: الإضافات
description: اكتب إضافاتك الخاصة لتوسيع WebForge.
---

تتيح لك الإضافات توسيع WebForge عبر ربطها بأحداث مختلفة وتخصيص السلوك. يمكنك إنشاء إضافات لإضافة ميزات جديدة، أو التكامل مع خدمات خارجية، أو تعديل السلوك الافتراضي لـ WebForge.
...
- `.webforge/plugins/` - إضافات على مستوى المشروع
- `~/.config/webforge/plugins/` - إضافات عامة
...
**إضافات npm** تُثبَّت تلقائيا باستخدام Bun عند بدء التشغيل. تُخزَّن الحزم واعتمادياتها مؤقتا في `~/.cache/webforge/node_modules/`.
...
1. إعدادات عامة (`~/.config/webforge/webforge.json`)
2. إعدادات المشروع (`webforge.json`)
3. دليل الإضافات العام (`~/.config/webforge/plugins/`)
4. دليل إضافات المشروع (`.webforge/plugins/`)
...
يشغّل WebForge الأمر `bun install` عند بدء التشغيل لتثبيتها. بعد ذلك يمكن لإضافاتك وأدواتك استيرادها.
...
- `client`: عميل SDK لـ webforge للتفاعل مع الذكاء الاصطناعي.
...
فيما يلي بعض أمثلة الإضافات التي يمكنك استخدامها لتوسيع webforge.
...
:::note
إذا كنت تستخدم تطبيق WebForge لسطح المكتب، فيمكنه إرسال إشعارات النظام تلقائيا عند جاهزية الرد أو عند حدوث خطأ في جلسة.
:::
...
امنع webforge من قراءة ملفات `.env`:
...
يمكن للإضافات أيضا إضافة أدوات مخصصة إلى webforge:
...
تقوم الدالة المساعدة `tool` بإنشاء أداة مخصصة يمكن لـ webforge استدعاؤها. تأخذ دالة مخطط Zod وتعيد تعريف أداة يتضمن:

- `description`: ما الذي تفعله الأداة
- `args`: مخطط Zod لوسائط الأداة
- `execute`: الدالة التي تُشغَّل عند استدعاء الأداة

ستكون أدواتك المخصصة متاحة لـ webforge إلى جانب الأدوات المضمنة.
