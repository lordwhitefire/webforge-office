...
أنشئ الملف `.webforge/commands/test.md`:
...
يمكنك إضافة أوامر مخصصة عبر إعدادات WebForge أو بإنشاء ملفات Markdown داخل الدليل `commands/`.

---

### JSON

استخدم خيار `command` في [config](/docs/config) الخاص بـ WebForge:
...
- على مستوى النظام: `~/.config/webforge/commands/`
- لكل مشروع: `.webforge/commands/`
...
يتضمن webforge عدة أوامر مضمنة مثل `/init` و`/undo` و`/redo` و`/share` و`/help`؛ [اعرف المزيد](/docs/tui#commands).
