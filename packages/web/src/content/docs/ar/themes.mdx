...
مع WebForge يمكنك الاختيار من بين عدة سمات مدمجة، أو استخدام سمة تتكيّف مع سمة terminal لديك، أو تعريف سمة مخصصة خاصة بك.

افتراضيًا، يستخدم WebForge سمتنا `webforge`.
...
يأتي WebForge مع عدة سمات مدمجة.
...
- يريدون أن يطابق WebForge مظهر terminal لديهم
...
يدعم WebForge نظام سمات مرنًا قائمًا على JSON يتيح للمستخدمين إنشاء السمات وتخصيصها بسهولة.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/webforge/themes/*.json` أو `$XDG_CONFIG_HOME/webforge/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.webforge/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.webforge/themes/*.json`
