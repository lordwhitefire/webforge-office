...
O WebForge Go é uma assinatura de baixo custo — **US$ 5 no seu primeiro mês**, depois **US$ 10/mês** — que oferece acesso confiável a modelos abertos de programação populares.

O Go funciona como qualquer outro provedor no WebForge. Você assina o WebForge Go e
...
O WebForge Go lhe dá acesso a esses modelos por **US$ 5 no seu primeiro mês**, depois **US$ 10/mês**.

---

## Como funciona

O WebForge Go funciona como qualquer outro provedor no WebForge.

1. Você entra no **<a href={console}>WebForge Zen</a>**, assina o Go e
   copia a sua chave de API.
2. Você executa o comando `/connect` na TUI, seleciona `WebForge Go` e cola
   a sua chave de API.
3. Execute `/models` na TUI para ver a lista de modelos disponíveis através do Go.

:::note
Apenas um membro por workspace pode assinar o WebForge Go.
:::
...
O WebForge Go inclui os seguintes limites:
...
- Kimi K2.7 Code/K2.6 — 870 tokens de entrada, 55.000 em cache, 200 tokens de saída por requisição
...
O [ID do modelo](/docs/config/#models) na sua configuração do WebForge
usa o formato `webforge-go/<model-id>`. Por exemplo, para o Kimi K2.7 Code, você usaria
`webforge-go/kimi-k2.7-code` na sua configuração.
...
4. Não ter **nenhuma restrição de fornecedor (lock-in)**, permitindo que você use qualquer outro provedor com o WebForge também.
