---
title: Provedores
description: Usando qualquer provedor LLM no webforge.
---

import config from "../../../../config.mjs"
export const console = config.console

webforge usa o [AI SDK](https://ai-sdk.dev/) e [Models.dev](https://models.dev) para suportar **75+ provedores LLM** e suporta a execução de modelos locais.

Para adicionar um provedor, você precisa:

1. Adicionar as chaves da API para o provedor usando o comando `/connect`.
2. Configurar o provedor na sua configuração do webforge.

---

### Credenciais

Quando você adiciona as chaves da API de um provedor com o comando `/connect`, elas são armazenadas em `~/.local/share/webforge/auth.json`.

---

### Configuração

Você pode personalizar os provedores através da seção `provider` na sua configuração do webforge.
...
WebForge Zen é uma lista de modelos fornecidos pela equipe do webforge que foram testados e verificados para funcionar bem com o webforge. [Saiba mais](/docs/zen).

:::tip
Se você é novo, recomendamos começar com o WebForge Zen.
:::

1. Execute o comando `/connect` no TUI, selecione `WebForge Zen` e acesse [opencode.ai/zen](https://opencode.ai/zen).
...
Funciona como qualquer outro provedor no webforge e é completamente opcional.

---

## WebForge Go

WebForge Go é um plano de assinatura de baixo custo que fornece acesso confiável a modelos de codificação abertos populares fornecidos pela equipe do webforge que foram
testados e verificados para funcionar bem com o webforge.

1. Execute o comando `/connect` no TUI, selecione `WebForge Go` e acesse [opencode.ai/zen](https://opencode.ai/zen).
...
Para usar o Amazon Bedrock com o webforge:
...
2. **Configure a autenticação** usando um dos seguintes métodos:
   ...
   ***
   ...
   Defina uma dessas variáveis de ambiente ao executar o webforge:
   ...
   ***
   ...
   Para configuração específica do projeto ou persistente, use `webforge.json`:
...
:::info
Usar sua assinatura Claude Pro/Max no webforge não é oficialmente suportado pela [Anthropic](https://anthropic.com).
:::
...
Você pode configurar o webforge para usar modelos locais através do [Atomic Chat](https://atomic.chat), um aplicativo de desktop que executa LLMs locais por trás de um servidor API compatível com OpenAI (endpoint padrão `http://127.0.0.1:1337/v1`).
...
2. Vá para [Azure AI Foundry](https://ai.azure.com/) e implante um modelo.
   ...
   :::note
   O nome da implantação deve corresponder ao nome do modelo para que o webforge funcione corretamente.
   :::
...
5. Execute o comando `/models` para selecionar um modelo.
   ...
   Você também pode adicionar modelos através da sua configuração do webforge.
...
armazenar o token no armazenamento de autenticação do webforge.
:::

##### GitLab Auto-Hospedado

:::note[nota de conformidade]
webforge usa um pequeno modelo para algumas tarefas de AI, como gerar o título da sessão.
Ele está configurado para usar gpt-5-nano por padrão, hospedado pela Zen. Para bloquear o webforge
para usar apenas sua própria instância hospedada do GitLab, adicione o seguinte ao seu
arquivo `webforge.json`. Também é recomendado desativar o compartilhamento de sessões.
...
Mais documentação na página [webforge-gitlab-auth](https://www.npmjs.com/package/webforge-gitlab-auth).

##### Configuração

Personalize através do `webforge.json`:
...
Para usar sua assinatura do GitHub Copilot com o webforge:
...
Para usar o Google Vertex AI com o webforge:
...
2. Defina as variáveis de ambiente necessárias:
   ...
   Defina-as ao executar o webforge.
...
Caso você veja um recurso ou modelo do Helicone que não está configurado automaticamente através do webforge, você sempre pode configurá-lo você mesmo.
...
O recurso [Sessões](https://docs.helicone.ai/features/sessions) do Helicone permite que você agrupe solicitações LLM relacionadas. Use o plugin [webforge-helicone-session](https://github.com/H2Shami/webforge-helicone-session) para registrar automaticamente cada conversa do webforge como uma sessão no Helicone.
...
O plugin injeta os cabeçalhos `Helicone-Session-Id` e `Helicone-Session-Name` em suas solicitações. Na página de Sessões do Helicone, você verá cada conversa do webforge listada como uma sessão separada.
...
Você pode configurar o webforge para usar modelos locais através da utilidade llama-server do [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Você pode configurar o webforge para usar modelos locais através do LM Studio.
...
Você pode configurar o webforge para usar modelos locais através do Ollama.

:::tip
Ollama pode se configurar automaticamente para o webforge. Consulte a [documentação de integração do Ollama](https://docs.ollama.com/integrations/webforge) para detalhes.
:::
...
Para usar o Ollama Cloud com o webforge:
...
3. Copie a chave da API para uso no webforge.
...
6. **Importante**: Antes de usar modelos em nuvem no webforge, você deve puxar as informações do modelo localmente:
...
WebForge Zen é uma lista de modelos testados e verificados fornecidos pela equipe do webforge. [Saiba mais](/docs/zen).

1. Faça login em **<a href={console}>WebForge Zen</a>** e clique em **Criar Chave da API**.

2. Execute o comando `/connect` e procure por **WebForge Zen**.

   ```txt
   /connect
   ```

3. Insira sua chave da API do webforge.
...
4. Muitos modelos do OpenRouter são pré-carregados por padrão, execute o comando `/models` para selecionar o que você deseja.
   ...
   Você também pode adicionar modelos adicionais através da sua configuração do webforge.
...
5. Você também pode personalizá-los através da sua configuração do webforge. Aqui está um exemplo de especificar um provedor
...
Para usar [APIs Generativas Scaleway](https://www.scaleway.com/en/docs/generative-apis/) com o webforge:
...
Você também pode personalizar modelos através da sua configuração do webforge. Aqui está um exemplo de especificar a ordem de roteamento do provedor.
...
:::tip
Você pode usar qualquer provedor compatível com OpenAI com o webforge. A maioria dos provedores de AI modernos oferece APIs compatíveis com OpenAI.
:::
...
3. Insira sua chave da API para o provedor.
   ...
   ┌  Adicionar credencial
   │
   ▲  Isso armazena apenas uma credencial para myprovider - você precisará configurá-lo no webforge.json, verifique a documentação para exemplos.
   │
...
4. Crie ou atualize seu arquivo `webforge.json` no diretório do seu projeto:
...
Os campos `limit` permitem que o webforge entenda quanto contexto você tem disponível. Provedores padrão puxam isso automaticamente do models.dev.
...
1. **Verifique a configuração de autenticação**: Execute `webforge auth list` para ver se as credenciais
...
2. Para provedores personalizados, verifique a configuração do webforge e:
   - Certifique-se de que o ID do provedor usado no comando `/connect` corresponda ao ID na sua configuração do webforge.
