---
title: Plugins
description: Escreva seus próprios plugins para estender o webforge.
---

Plugins permitem que você estenda o webforge conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do webforge.
...
- `.webforge/plugins/` - Plugins em nível de projeto
- `~/.config/webforge/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/webforge/node_modules/`.
...
1. Configuração global (`~/.config/webforge/webforge.json`)
2. Configuração do projeto (`webforge.json`)
3. Diretório de plugins global (`~/.config/webforge/plugins/`)
4. Diretório de plugins do projeto (`.webforge/plugins/`)
...
O webforge executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do webforge para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o webforge.
...
:::note
Se você estiver usando o aplicativo desktop webforge, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o webforge de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao webforge:
...
O helper `tool` cria uma ferramenta personalizada que o webforge pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o webforge junto com as ferramentas integradas.
