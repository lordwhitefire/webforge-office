...
As habilidades do agente permitem que o webforge descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O webforge pesquisa nesses locais:

- Configuração do projeto: `.webforge/skills/<name>/SKILL.md`
- Configuração global: `~/.config/webforge/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o webforge sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.webforge/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/webforge/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.webforge/skills/git-release/SKILL.md` assim:
...
O webforge lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `webforge.json`:
...
**Para agentes embutidos** (em `webforge.json`):
