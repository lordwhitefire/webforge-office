---
title: Servidor
description: Interaja com o servidor webforge via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `webforge serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente webforge pode usar.
...
```bash
webforge serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `webforge.local` |
| `--cors`        | Origens adicionais de navegador a permitir        | `[]`             |

`--cors` pode ser passado várias vezes:

```bash
webforge serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Defina `WEBFORGE_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `webforge`, ou defina `WEBFORGE_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `webforge serve` quanto ao `webforge web`.

```bash
WEBFORGE_SERVER_PASSWORD=your-password webforge serve
```
...
Quando você executa `webforge`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor webforge para interagir com o webforge programaticamente.
:::

Essa arquitetura permite que o webforge suporte múltiplos clientes e permite que você interaja com o webforge programaticamente.

Você pode executar `webforge serve` para iniciar um servidor autônomo. Se você tiver o TUI do webforge em execução, `webforge serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do webforge [IDE](/docs/ide).
...
O servidor webforge expõe as seguintes APIs.
