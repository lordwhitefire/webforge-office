...
Com o webforge, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o webforge usa nosso próprio tema `webforge`.
...
O webforge vem com vários temas embutidos.
...
- Querem que o webforge corresponda à aparência do seu terminal
...
O webforge suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/webforge/themes/*.json` ou `$XDG_CONFIG_HOME/webforge/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.webforge/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.webforge/themes/*.json`
