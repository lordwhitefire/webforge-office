...
Du kan legge til eksterne verktøy i WebForge ved å bruke _Model Context Protocol_ eller MCP. WebForge støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [WebForge Config](https://opencode.ai/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="webforge.jsonc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/webforge`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
WebForge håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil WebForge:
...
Hvis serveren krever autentisering, vil WebForge be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `webforge mcp auth <server-name>`.
...
```bash
webforge mcp auth my-oauth-server
```
...
```bash
webforge mcp logout my-oauth-server
```

Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil WebForge lagre tokens sikkert i `~/.local/share/webforge/mcp-auth.json`.
...
webforge mcp debug my-oauth-server
```
...
MCP-ene dine er tilgjengelige som verktøy i WebForge, sammen med innebygde verktøy. Så du kan administrere dem gjennom WebForge-konfigurasjonen som alle andre verktøy.
...
```json title="webforge.json" {11, 14-18}
{
  "$schema": "https://opencode.ai/config.json",
  "mcp": {
    "my-mcp": {
...
```bash
webforge mcp auth sentry
```

Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble WebForge til Sentry-kontoen din.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver WebForge-samtale oppført som en separat økt.
