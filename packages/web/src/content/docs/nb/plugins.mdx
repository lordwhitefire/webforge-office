...
description: Skriv dine egne programtillegg for å utvide WebForge.
---

Plugins lar deg utvide WebForge ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til WebForge.
...
- `.webforge/plugins/` - Programtillegg på prosjektnivå
...
**npm-plugins** installeres automatisk ved hjelp av Bun ved oppstart. Pakker og deres avhengigheter er bufret i `~/.cache/webforge/node_modules/`.
...
1. Global konfigurasjon (`~/.config/webforge/webforge.json`)
2. Prosjektkonfigurasjon (`webforge.json`)
3. Global plugin-katalog (`~/.config/webforge/plugins/`)
4. Prosjektpluginkatalog (`.webforge/plugins/`)
...
WebForge kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En WebForge SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide WebForge.
...
:::note
Hvis du bruker WebForge-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre WebForge fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til WebForge:
...
`tool`-hjelperen lager et tilpasset verktøy som WebForge kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for WebForge sammen med innebygde verktøy.
