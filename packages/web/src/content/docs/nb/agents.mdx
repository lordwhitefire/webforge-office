...
Det er to typer agenter i WebForge; primære agenter og underagenter.
...
WebForge kommer med to innebygde primære agenter, **Build** og **Plan**. Vi ser nærmere på disse nedenfor.
...
WebForge kommer med tre innebygde underagenter, **General**, **Explore** og **Scout**. Vi skal se på dette nedenfor.

---

## Innebygd

WebForge kommer med to innebygde primære agenter og tre innebygde underagenter.
...
Konfigurer agenter i konfigurasjonsfilen `webforge.json`:
...
Hvis ingen temperatur er spesifisert, bruker WebForge modellspesifikke standardinnstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne banen er i forhold til hvor konfigurasjonsfilen er plassert. Så dette fungerer for både den globale WebForge-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
...
Modellen ID i WebForge-konfigurasjonen din bruker formatet `provider/model-id`. For eksempel, hvis du bruker [WebForge Zen](/docs/zen), vil du bruke `webforge/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Kjør `webforge models` for å se en liste over tilgjengelige modeller.
:::
...
:::tip
Har du en agent du vil dele? [Send inn en PR](https://github.com/anomalyco/webforge).
:::
