---
title: Tjener
description: Samhandle med WebForge-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `webforge serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en WebForge-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `webforge.local` |
...
Sett `WEBFORGE_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `webforge` som standard, eller sett `WEBFORGE_SERVER_USERNAME` for å overstyre det. Dette gjelder både `webforge serve` og `webforge web`.
...
Når du kjører `webforge` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk WebForge-serveren til å samhandle med WebForge programmatisk.
:::

Denne arkitekturen lar WebForge støtte flere klienter og lar deg samhandle med WebForge programmatisk.

Du kan kjøre `webforge serve` for å starte en frittstående server. Hvis du har
WebForge TUI kjører, vil `webforge serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av WebForge [IDE](/docs/ide) plugins.
...
webforge-serveren viser følgende APIer.
