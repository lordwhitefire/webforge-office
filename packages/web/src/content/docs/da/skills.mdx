...
Agentferdigheter lar WebForge oppdage gjenbrugbare instruksjoner fra repo- eller hjemmekatalogen din.
...
WebForge søker etter disse stedene:

- Prosjektkonfigurasjon: `.webforge/skills/<name>/SKILL.md`
- Global konfigurasjon: `~/.config/webforge/skills/<name>/SKILL.md`
...
For prosjektlokale stier går WebForge opp fra din nåværende arbejdskatalog til den når git-arbeidstreet.
Den laster alle matchende `skills/*/SKILL.md` i `.webforge/` og alle matchende `.claude/skills/*/SKILL.md` eller `.agents/skills/*/SKILL.md` underveis.

Globale definisjoner lastes også inn fra `~/.config/webforge/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` og `~/.agents/skills/*/SKILL.md`.
...
Lag `.webforge/skills/git-release/SKILL.md` slik:
...
WebForge viser tilgængelige ferdigheter i `skill` verktøybeskrivelsen.
...
Kontroller hvilke ferdigheter agenter har tilgang til ved at bruge mønsterbaserte tillatelser i `webforge.json`:
...
**For innebygde agenter** (i `webforge.json`):
