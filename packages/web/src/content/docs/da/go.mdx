...
WebForge Go er et lavprisabonnement — **$5 for din første måned**, derefter **$10/måned** — der giver dig pålidelig adgang til populære åbne kodningsmodeller.

Go fungerer som enhver anden udbyder i WebForge. Du abonnerer på WebForge Go og
...
:::tip
Vi testede en udvalgt gruppe af modeller og udbydere, der fungerer godt med WebForge.
:::
...
WebForge Go giver dig adgang til disse modeller for **$5 for din første måned**, derefter **$10/måned**.

---

## Sådan fungerer det

WebForge Go fungerer som enhver anden udbyder i WebForge.

1. Du logger ind på **<a href={console}>WebForge Zen</a>**, abonnerer på Go, og
   kopierer din API-nøgle.
2. Du kører kommandoen `/connect` i TUI'en, vælger `WebForge Go`, og indsætter
   din API-nøgle.
3. Kør `/models` i TUI'en for at se listen over tilgængelige modeller gennem Go.

:::note
Kun ét medlem per arbejdsområde kan abonnere på WebForge Go.
:::
...
WebForge Go inkluderer følgende grænser:
...
- Kimi K2.7 Code/K2.6 — 870 input, 55.000 cachelagrede, 200 output-tokens pr. anmodning
...
Dit [model id](/docs/config/#models) i din WebForge config
bruger formatet `webforge-go/<model-id>`. For eksempel for Kimi K2.7 Code, vil du
bruge `webforge-go/kimi-k2.7-code` i din config.
...
4. Undgå **lock-in** ved også at lade dig bruge enhver anden udbyder med WebForge.
