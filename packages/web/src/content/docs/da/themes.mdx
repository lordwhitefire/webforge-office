...
Med WebForge kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger WebForge vårt eget `webforge`-tema.
...
WebForge kommer med flere innebygde temaer.
...
- Vil at WebForge skal matche terminalens utseende
...
WebForge støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/webforge/themes/*.json` eller `$XDG_CONFIG_HOME/webforge/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.webforge/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.webforge/themes/*.json`
