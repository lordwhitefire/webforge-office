---
title: Plugins
description: Skriv dine egne plugins for at udvide WebForge.
---

Plugins giver dig mulighed for at udvide WebForge ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre WebForges standardadfærd.
...
- `.webforge/plugins/` - Plugins på projektniveau
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/webforge/node_modules/`.
...
1. Global konfiguration (`~/.config/webforge/webforge.json`)
2. Projektkonfiguration (`webforge.json`)
3. Global plugin-mappe (`~/.config/webforge/plugins/`)
4. Projekt plugin bibliotek (`.webforge/plugins/`)
...
WebForge kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En webforge SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide webforge.
...
:::note
Hvis du bruger WebForge desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå webforge i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til webforge:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som webforge kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for webforge sammen med indbyggede værktøjer.
