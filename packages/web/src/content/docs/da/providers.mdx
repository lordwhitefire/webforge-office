---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i WebForge.
---

import config from "../../../../config.mjs"
export const console = config.console

WebForge bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i WebForge-konfigurationen.

---

### Legitimationsoplysninger

Når du tilføjer en udbyders API-nøgler med `/connect`-kommandoen, gemmes de i `~/.local/share/webforge/auth.json`.

---

### Konfiguration

Du kan tilpasse udbyderne gennem `provider`-delen i WebForge-konfigurationen.
...
WebForge Zen er en liste over modeller leveret af WebForge-teamet, som er blevet testet og verificeret til at fungere godt med WebForge. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med WebForge Zen.
:::

1. Kør kommandoen `/connect` i TUI, vælg webforge og gå til [opencode.ai/auth](https://opencode.ai/auth).
...
Det fungerer som alle andre udbydere i WebForge og er helt valgfrit at bruge.

---

## WebForge Go

WebForge Go er en billig abonnementsplan, der giver pålidelig adgang til populære åbne kodningsmodeller leveret af WebForge-teamet, som er testet og verificeret til at fungere godt med WebForge.

1. Kør kommandoen `/connect` i TUI, vælg `WebForge Go`, og gå til [opencode.ai/auth](https://opencode.ai/zen).
...
Sådan bruger du Amazon Bedrock med WebForge:
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   Angiv en af disse miljøvariabler, mens du kører webforge:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `webforge.json`:
...
:::info
At bruge dit Claude Pro/Max-abonnement i WebForge understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere webforge til at bruge lokale modeller via [Atomic Chat](https://atomic.chat) — en desktopapplikation, der kører lokale LLM'er bag en OpenAI-kompatibel API-server (standard-endpoint `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en model.
   ...
   :::note
   Distributionsnavnet skal matche modelnavnet for at webforge skal fungere korrekt.
   :::
...
5. Kør kommandoen `/models` for at vælge en model.
   ...
   Du kan også tilføje modeller gennem webforge-konfigurationen.
...
gemme tokenet i webforge auth-lagring.
:::

##### Selvhostet GitLab

:::note[overholdelsesbemærkning]
WebForge bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse WebForge
til kun at bruge din egen GitLab-hostede instans, tilføj følgende i din
`webforge.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
Mere dokumentation på [webforge-gitlab-auth](https://www.npmjs.com/package/webforge-gitlab-auth) hjemmesiden.

##### Konfiguration

Tilpas gennem `webforge.json`:
...
Sådan bruger du GitHub Copilot-abonnementet med WebForge:
...
Sådan bruger du Google Vertex AI med WebForge:
...
2. Angiv de nødvendige miljøvariabler:
   ...
   Sæt dem mens du kører webforge.
...
I tilfælde af at du ser en funktion eller model fra Helicone som ikke konfigureres automatisk gennem webforge, kan du altid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [webforge-helicone-session](https://github.com/H2Shami/webforge-helicone-session) plugin for automatisk at logge hver WebForge-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver WebForge-samtale opført som en separat session.
...
Du kan konfigurere webforge til at bruge lokale modeller gennem [llama.cpps](https://github.com/ggml-org/llama.cpp) llama-server-værktøj
...
Du kan konfigurere webforge til at bruge lokale modeller gennem LM Studio.
...
Du kan konfigurere webforge til at bruge lokale modeller gennem Ollama.

:::tip
Ollama kan automatisk konfigurere sig selv for WebForge. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/webforge) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med WebForge:
...
3. Kopier API-nøglen til brug i WebForge.
...
6. **Vigtigt**: Før du bruger skymodeller i WebForge, skal du hente modelinformationen lokalt:
...
WebForge Zen er en liste over testede og verificerede modeller leveret af WebForge-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>WebForge Zen</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **WebForge Zen**.

   ```txt
   /connect
   ```

3. Indtast WebForge API-nøglen.
...
4. Mange OpenRouter-modeller er forudindlæst som standard, kør kommandoen `/models` for at vælge den du ønsker.
   ...
   Du kan også tilføje flere modeller gennem webforge-konfigurationen.
...
5. Du kan også tilpasse dem gennem webforge-konfigurationen. Her er et eksempel på at specificere en udbyder
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med WebForge:
...
Du kan også tilpasse modeller gennem webforge-konfigurationen. Her er et eksempel på specificering af udbyder-rutingsrækkefølge.
...
:::tip
Du kan bruge hvilken som helst OpenAI-kompatibel udbyder med webforge. De fleste moderne AI-udbydere tilbyder OpenAI-kompatible API'er.
:::
...
4. Opret eller opdater `webforge.json`-filen i projektkataloget:
...
`limit`-felterne lader WebForge forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
...
1. **Tjek godkendelsesopsætningen**: Kør `webforge auth list` for at se om legitimationsoplysningerne
...
2. For tilpassede udbydere, tjek webforge-konfigurationen og:
   - Sørg for at udbyderens ID som bruges i `/connect`-kommandoen matcher ID i webforge-konfigurationen.
