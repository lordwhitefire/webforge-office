...
Her er alle de innebygde verktøyene som er tilgængelige i WebForge.
...
:::note
Dette verktøyet er bare tilgængelig når `WEBFORGE_EXPERIMENTAL_LSP_TOOL=true` (eller `WEBFORGE_EXPERIMENTAL=true`).
:::
...
:::note
Dette verktøyet er bare tilgængelig når du bruger WebForge-leverandøren eller når miljøvariabelen `WEBFORGE_ENABLE_EXA` er satt til en sannverdi (f.eks. `true` eller `1`).
