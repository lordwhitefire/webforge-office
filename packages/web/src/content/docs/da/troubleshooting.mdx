...
For at fejlfinde problemer med WebForge, start med at tjekke logfilerne og de lokale data, den gemmer på disken.
...
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\webforge\log`

Logfiler navngives med tidsstempler (f.eks. `2025-01-09T123456.log`) og de sidste 10 logfiler beholdes.

Du kan angive logniveauet med kommandolinjeflaget `--log-level` for at få mere detaljeret fejlfindingsinformation. For eksempel `webforge --log-level DEBUG`.

---

## Lagring

webforge gemmer sessionsdata og andre applikationsdata på disken på:

- **macOS/Linux**: `~/.local/share/webforge/`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\webforge`
...
WebForge Desktop kører en lokal WebForge-server (`webforge-cli` sidevognen) i baggrunden. De fleste problemer er forårsaget af et plugin, der fungerer dårligt, en ødelagt cache eller en dårlig serverindstilling.

### Hurtige tjek

- Afslut og start appen på ny.
- Hvis appen viser en fejlskærm, klikker du på **Start på ny** og kopierer fejldetaljerne.
- Kun macOS: `WebForge`-menu -> **Genindlæs webvisning** (hjælper hvis UI er tom/frosset).
...
- **macOS/Linux**: `~/.config/webforge/webforge.jsonc` (eller `~/.config/webforge/webforge.json`)
- **macOS/Linux** (ældre installationer): `~/.local/share/webforge/webforge.jsonc`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\webforge\webforge.jsonc`
...
WebForge kan også indlæse lokale plugins fra disken. Flyt disse midlertidigt væk (eller giv mappen nyt navn) og start skrivebordsappen på ny:

- **Globale plugins**
  - **macOS/Linux**: `~/.config/webforge/plugins/`
  - **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\webforge\plugins`
...
Hvis deaktivering af plugins ikke hjælper (eller en plugin-installation sidder fast), tøm cachen så WebForge kan genopbygge den.

1. Afslut WebForge Desktop helt.
2. Slet cache-mappen:

- **macOS**: Finder -> `Cmd+Shift+G` -> indsæt `~/.cache/webforge`
- **Linux**: slet `~/.cache/webforge` (eller kør `rm -rf ~/.cache/webforge`)
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.cache\webforge`

3. Start WebForge Desktop på ny.

---

### Løs problemer med serverforbindelse

WebForge Desktop kan enten starte sin egen lokale server (standard) eller forbinde til en server URL, du har konfigureret.
...
Hvis `webforge.json(c)` indeholder en `server`-del, fjern den midlertidigt og start skrivebordsappen på ny.

#### Tjek miljøvariabler

Hvis du har `WEBFORGE_PORT` sat i dit miljø, vil skrivebordsappen prøve at bruge den port for den lokale server.

- Deaktiver `WEBFORGE_PORT` (eller vælg en ledig port) og start på ny.
...
På Windows kræver WebForge Desktop Microsoft Edge **WebView2 Runtime**. Hvis appen åbnes i et tomt vindue eller ikke starter, installer/opdater WebView2 og prøv igen.

---

### Windows: Generelle ydeevneproblemer

Hvis du oplever langsom ydeevne, problemer med filadgang eller terminalproblemer på Windows, kan du prøve at bruge [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL giver et Linux-miljø som fungerer mere sømløst med funktionerne i WebForge.

---

### Meddelelser vises ikke

WebForge Desktop viser kun systemvarsler når:

- varsler er aktiveret for WebForge i dine OS-indstillinger, og
...
1. Afslut WebForge Desktop.
2. Find og slet disse filer (de findes i WebForge Desktop-appens datamappe):

- `webforge.settings.dat` (skrivebordsstandardserver URL)
- `webforge.global.dat` og `webforge.workspace.*.dat` (UI tilstand som nylige servere/projekter)
...
Hvis du oplever problemer med WebForge:
...
For at finde ud af hvilke modeller du har adgang til, kør `webforge models`
...
2. Hvis problemet vedvarer, prøv at tømme den gemte konfiguration:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.local\share\webforge`
...
Hvis du støder på API-kaldsfejl, kan dette skyldes forældede udbyderpakker. webforge installerer dynamisk udbyderpakker (OpenAI, Anthropic, Google, etc.) efter behov og cacher dem lokalt.
...
1. Tøm udbyderens pakkecache:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.cache\webforge`

2. Start webforge på ny for at installere de nyeste udbyderpakker på ny

Dette vil tvinge webforge til at downloade de nyeste versioner af udbyderpakkerne, som ofte løser kompatibilitetsproblemer med modelparametre og API-ændringer.
...
webforge vil opdage om du bruger Wayland og foretrække `wl-clipboard`, ellers vil den prøve at finde udklipsholderværktøjer i rækkefølgen: `xclip` og `xsel`.
