---
title: Konfiguration
description: Ved at bruge WebForge JSON konfig.
---

Du kan konfigurere WebForge ved hjælp af en JSON-konfigurationsfil.

---

## Format

WebForge understøtter både **JSON** og **JSONC** (JSON med kommentarer) formater.
...
1. **Fjernkonfiguration** (fra `.well-known/webforge`) - organisatoriske standardindstillinger
2. **Global config** (`~/.config/webforge/webforge.json`) - brugerpræferencer
3. **Tilpasset konfiguration** (`WEBFORGE_CONFIG` env var) - tilpassede tilsidesættelser
4. **Project config** (`webforge.json` i projekt) - projektspecifikke indstillinger
5. **`.webforge` mapper** - agent, kommandoer, plugins
6. **Inline config** (`WEBFORGE_CONFIG_CONTENT` env var) - runtime tilsidesættelser
...
:::note
`.webforge` og `~/.config/webforge` mapperne bruger **flertalsnavne** for undermapper: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` og `themes/`. Enkelte navne (f.eks. `agent/`) understøtter også bagudkompatibilitet.
:::
...
Organisationer kan levere standardkonfiguration via `.well-known/webforge`-slutpunktet. Dette hentes automatisk, når du godkender med en udbyder, der understøtter det.
...
Placer din globale WebForge-konfiguration i `~/.config/webforge/webforge.json`. Brug global konfiguration til brugerdækkende præferencer som temaer, udbydere eller nøglebindinger.
...
Tilføj `webforge.json` i dit projektrod. Project config har den højeste forrang blandt standard config-filer - den tilsidesætter både globale og eksterne config.

:::tip
Placer projektspecifik konfiguration i roden af ​​dit projekt.
:::

Når WebForge starter op, søger den efter en konfigurationsfil i den aktuelle mappe eller går op til den nærmeste Git-mappe.
...
Angiv en brugerdefineret konfigurationsfilsti ved hjælp af miljøvariablen `WEBFORGE_CONFIG`.
...
Angiv en brugerdefineret konfigurationsmappe ved hjælp af `WEBFORGE_CONFIG_DIR`
miljøvariabel. Dette kort vil blive søgt efter agenter, kommandoer,
modes og plugins ligesom standard `.webforge` mappen, og bør
...
Den brugerdefinerede mappe indlæses efter den globale konfig og `.webforge` mapper, så den **kan tilsidesætte** deres indstillinger.
...
Du kan konfigurere serverindstillinger for kommandoerne `webforge serve` og `webforge web` gennem indstillingen `server`.
...
- `mdns` - Aktiver mDNS-tjenesteopdagelse. Dette gør det muligt for andre enheder på netværket at opdage din WebForge-server.
- `mdnsDomain` - Brugerdefineret domænenavn til mDNS-tjeneste. Som standard er `webforge.local`. Nyttigt til at køre flere forekomster på det samme netværk.
...
Du kan konfigurere de udbydere og modeller, du vil bruge i din WebForge-konfiguration, gennem mulighederne `provider`, `model` og `small_model`.
...
Indstillingen `small_model` konfigurerer en separat model til lette opgaver som titelgenerering. Som standard forsøger WebForge at bruge en billigere model, hvis en er tilgængelig fra din udbyder, ellers falder den tilbage til din hovedmodel.
...
Du kan konfigurere det tema, du vil bruge i din WebForge-konfiguration, gennem indstillingen `theme`.
...
Du kan også definere agenter ved at bruge markdown-filer i `~/.config/webforge/agents/` eller `.webforge/agents/`. [Learn more here](/docs/agents).
...
Standardagenten skal være en primær agent (ikke en subagent). Dette kan være en indbygget agent som `"build"` eller `"plan"`, eller en [custom agent](/docs/agents), du har defineret. Hvis den angivne agent ikke eksisterer eller er en underagent, vil WebForge falde tilbage til `"build"` med en advarsel.

Denne indstilling gælder på tværs af alle grænseflader: TUI, CLI (`webforge run`), desktop-app og GitHub Action.
...
Du kan også definere kommandoer ved hjælp af markdown-filer i `~/.config/webforge/commands/` eller `.webforge/commands/`. [Learn more here](/docs/commands).
...
WebForge vil automatisk downloade alle nye opdateringer, når den starter op. Du kan deaktivere dette med indstillingen `autoupdate`.
...
Som standard webforge **tillader alle operationer** uden at kræve eksplicit godkendelse. Du kan ændre dette ved at bruge indstillingen `permission`.
...
[Plugins](/docs/plugins) udvide WebForge med brugerdefinerede værktøjer, kroge og integrationer.

Placer plugin-filer i `.webforge/plugins/` eller `~/.config/webforge/plugins/`. Du kan også indlæse plugins fra npm gennem indstillingen `plugin`.
...
Dette er nyttigt, når du vil begrænse WebForge til kun at bruge specifikke udbydere i stedet for at deaktivere dem én efter én.
