...
import { SessionStatusEvent } from "@webforge-ai/schema/session-status-event"
import { SessionV1 } from "@webforge-ai/schema/session-v1"
import type {
  AssistantMessage,
  GlobalEvent,
  Message,
...
export const directory = "C:/WebForge/TimelineStability"
...
export const model = { providerID: "webforge", modelID: "claude-opus-4-6", variant: "max" }
...
export async function setupTimeline(
  ...
  if (input.locale) {
    await page.addInitScript((locale) => {
      localStorage.setItem("webforge.global.dat:language", JSON.stringify({ locale }))
    }, input.locale)
  }
...
function provider() {
  return {
    ...
    connected: ["webforge"],
    default: { providerID: "webforge", modelID: "claude-opus-4-6" },
  }
}
