...
const envPath = process.env.WEBFORGE_BIN_PATH
const scriptDir = path.dirname(fs.realpathSync(__filename))
const cached = path.join(scriptDir, ".webforge")
const platform = { darwin: "darwin", linux: "linux", win32: "windows" }[os.platform()] || os.platform()
const arch = { x64: "x64", arm64: "arm64", arm: "arm" }[os.arch()] || os.arch()
const base = "@webforge-ai/cli-" + platform + "-" + arch
const binary = platform === "windows" ? "webforge.exe" : "webforge"
...
if (!resolved) {
  console.error(
    "It seems that your package manager failed to install the right webforge CLI package. Try manually installing " +
