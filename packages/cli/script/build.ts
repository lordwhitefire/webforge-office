...
for (const item of targets) {
  ...
  const result = await Bun.build({
    ...
    define: {
      ...
      'webforge': `'${binary}'`,
      WEBFORGE_MODELS_DEV: modelsData,
      WEBFORGE_CHANNEL: `'${Script.channel}'`,
      WEBFORGE_LIBC: item.os === "linux" ? `'${item.abi ?? "glibc"}'` : "undefined",
  ...
  await Bun.write(
    ...
    JSON.stringify(
      {
        name: `@webforge-ai/${name}`,
        version: Script.version,
        license: "MIT",
        repository: { type: "git", url: "git+https://github.com/anomalyco/webforge.git" },
        os: [item.os],
        cpu: [item.arch],
      },
      null,
