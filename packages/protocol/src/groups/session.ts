...
import { PromptInput } from "@webforge-ai/schema/prompt-input"
import { Session } from "@webforge-ai/schema/session"
import { Project } from "@webforge-ai/schema/project"
import { AbsolutePath, NonNegativeInt, PositiveInt, RelativePath, statics } from "@webforge-ai/schema/schema"
...
import { SessionEvent } from "@webforge-ai/schema/session-event"
    ...
    .add(
      HttpApiEndpoint.get("session.active", "/api/session/active", {
        ...
        OpenApi.annotations({
          ...
          description:
            "Retrieve foreground Session drains currently owned by this WebForge process. Sessions absent from the result are inactive.",
        }),
      ),
    )
    .add(
      ...
      HttpApiEndpoint.post("session.interrupt", "/api/session/:sessionID/interrupt", {
        ...
        .annotateMerge(
          OpenApi.annotations({
            ...
            description: "Interrupt active execution owned by this WebForge process. Idle interruption is a no-op.",
          }),
        ),
    )
    .add(
