...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "webforge.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "webforge.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "webforge.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "webforge.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["webforge.json", "webforge.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "webforge.json"),
        path.join(project, "webforge.jsonc"),
      ])
    })
  })
