...
import { createWebforge, type ToolPart } from "@webforge-ai/sdk"
...
console.log("🚀 Starting webforge server...")
const webforge = await createWebforge({
  port: 0,
})
console.log("✅ Webforge server ready")

const sessions = new Map<string, { client: any; server: any; sessionId: string; channel: string; thread: string }>()
void (async () => {
  const events = await webforge.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new webforge session...")
    const { client, server } = webforge
    ...
    console.log("✅ Created webforge session:", createResult.data.id)
  ...
  console.log("📝 Sending to webforge:", message.text)
  const result = await session.client.session.prompt({
    path: { id: session.sessionId },
    body: { parts: [{ type: "text", text: message.text }] },
  })

  console.log("📤 Webforge response:", JSON.stringify(result, null, 2))
