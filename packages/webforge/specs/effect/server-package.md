...
Practical reference for a future `packages/server` split after the webforge
server moved to the Effect HttpApi backend.

## Current State

- The server still lives in `packages/webforge`.
...
Until enough shared service code lives outside `packages/webforge`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/webforge`

It should not import `packages/webforge` services while `packages/webforge`
...
2. Move stable domain schemas into shared packages only when they no longer
   depend on webforge-local runtime modules.
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/webforge` implementation details.
