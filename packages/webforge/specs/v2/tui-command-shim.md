...
Delete `packages/webforge/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/webforge/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/webforge/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/webforge`
- TUI plugin loader tests in `packages/webforge` if runtime plugin API wiring changed
