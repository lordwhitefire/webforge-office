...
- Author package entrypoint is `@webforge-ai/plugin/tui`.
...
```json
{
  ...
  "plugin": ["@acme/webforge-plugin@1.2.3", ["./plugins/demo.tsx", { "label": "demo" }]],
  "plugin_enabled": {
    "acme.demo": false
  },
  "attention": {
...
- Import types from `@webforge-ai/plugin/tui`.
- `@webforge-ai/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@webforge-ai/plugin/tui"
...
- The value is a semver range checked against the running WebForge version.
- If the range is not satisfied, the plugin is skipped with a warning and a session error.
- If `engines.webforge` is absent, no check is performed (backward compatible).
...
- `webforge plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `webforge plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.webforge` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.webforge`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.webforge`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.webforge` dir or `WEBFORGE_CONFIG_DIR`, WebForge installs `@webforge-ai/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@webforge-ai/plugin/tui`.
...
- The host already installs the default OpenTUI bundle (`default keys`, metadata fields, and enabled fields) plus WebForge's comma bindings, leader token, base layout fallback, pending-sequence helpers, and managed textarea layer.
...
WebForge registers a `mode` layer field on the host keymap. Plugins can use it to keep bindings active only in the relevant UI state.
...
Mode pushes are automatically tracked by the plugin runtime. If a plugin is disabled, fails during activation, or the TUI shuts down before the plugin calls the disposer, WebForge pops the plugin's pushed modes during plugin cleanup. Calling the disposer yourself is still recommended for component lifetimes; cleanup remains idempotent.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@webforge-ai/plugin/tui`.
...
- `message` is required; `title` defaults to `"webforge"`; `notification` defaults to enabled with `when: "blurred"`; `sound` defaults to enabled with `when: "always"`.
...
- Config `attention.sounds` overrides active-pack sounds by slot. Failed loads fall back to the active pack and then `webforge.default`.
...
- Local plugins persist installed themes under the local `.webforge/themes` area near the plugin config source.
...
- `--pure` / `WEBFORGE_PURE` skips external TUI plugins only.
...
- Local smoke plugin: `.webforge/plugins/tui-smoke.tsx`
- Local vim plugin: `.webforge/plugins/tui-vim.tsx`
- Local smoke config: `.webforge/tui.json`
- Local smoke theme: `.webforge/plugins/smoke-theme.json`
