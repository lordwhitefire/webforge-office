// Shared type vocabulary for the direct interactive mode (`webforge --mini`).
//
...
import type { WebforgeClient, PermissionRequest, QuestionRequest, ToolPart } from "@webforge-ai/sdk/v2"
import type { TuiConfig } from "@webforge-ai/tui/config"
...
type PromptModel = Parameters<WebforgeClient["session"]["prompt"]>[0]["model"]
type PromptInput = Parameters<WebforgeClient["session"]["prompt"]>[0]

export type RunPromptPart = NonNullable<PromptInput["parts"]>[number]

export type RunCommand = NonNullable<Awaited<ReturnType<WebforgeClient["command"]["list"]>>["data"]>[number]

export type RunProvider = NonNullable<Awaited<ReturnType<WebforgeClient["provider"]["list"]>>["data"]>["all"][number]
...
export type RunAgent = NonNullable<Awaited<ReturnType<WebforgeClient["app"]["agents"]>>["data"]>[number]

type RunResourceMap = NonNullable<Awaited<ReturnType<WebforgeClient["experimental"]["resource"]["list"]>>["data"]>
...
export type PermissionReply = Parameters<WebforgeClient["permission"]["reply"]>[0]

export type QuestionReply = Parameters<WebforgeClient["question"]["reply"]>[0]

export type QuestionReject = Parameters<WebforgeClient["question"]["reject"]>[0]
