import type { PermissionV1 } from "@webforge-ai/core/v1/permission"
import { FSUtil } from "@webforge-ai/core/fs-util"
// CLI entry point for `webforge run` and `webforge --mini`.
//
// Handles three modes:
//   1. Non-interactive (default): sends a single prompt, streams events to
//      stdout, and exits when the session goes idle.
//   2. Interactive local (`webforge --mini`): boots the split-footer direct mode
//      with an in-process server (no external HTTP).
//   3. Interactive attach (`webforge --mini --attach`): connects to a running
//      webforge server and runs interactive mode against it.
//
...
import { createWebforgeClient, type WebforgeClient, type ToolPart } from "@webforge-ai/sdk/v2"
import { FormatError, FormatUnknownError } from "../error"
import { INTERACTIVE_INPUT_ERROR, resolveInteractiveStdin } from "./run/runtime.stdin"

type ModelInput = Parameters<WebforgeClient["session"]["prompt"]>[0]["model"]
...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run webforge with a message",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running webforge server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to WEBFORGE_SERVER_PASSWORD)",
      })
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to WEBFORGE_SERVER_USERNAME or 'webforge')",
      })
      .option("dir", {
        type: "string",
  ...
  handler: Effect.fn("Cli.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      async function session(sdk: WebforgeClient): Promise<SessionInfo | undefined> {
        if (args.session) {
      ...
      async function share(sdk: WebforgeClient, sessionID: string) {
      ...
      async function current(sdk: WebforgeClient): Promise<string> {
        if (!args.attach) {
      ...
      async function attachAgent(sdk: WebforgeClient) {
        if (!args.agent) return undefined
        const name = args.agent
      ...
      async function pickAgent(sdk: WebforgeClient) {
        if (!args.agent) return undefined
        if (args.attach) {
      ...
      async function execute(sdk: WebforgeClient) {
        ...
        async function loop(client: WebforgeClient, events: Awaited<ReturnType<typeof sdk.event.subscribe>>) {
      ...
      const sdk = createWebforgeClient({
        baseUrl: "http://webforge.internal",
        fetch: fetchFn,
        directory,
      })
      await execute(sdk)
