...
import type { AssistantMessage, Message, WebforgeClient, SessionMessageResponse } from "@webforge-ai/sdk/v2"
...
export const AuthMethodID = "webforge-login"
...
export class Service extends Context.Service<Service, Interface>()("@webforge/ACP/Service") {}
...
export function make(input: {
  ...
  const initialize = Effect.fn("ACP.initialize")(function* (params: InitializeRequest) {
    ...
    const authMethod: AuthMethod = {
      description: "Run `webforge auth login` in the terminal",
      name: "Login with webforge",
      id: AuthMethodID,
    }
...
function makeDirectoryService(sdk: WebforgeClient) {
...
function makeUsageService(sdk: WebforgeClient) {
...
async function loadDirectorySnapshot(sdk: WebforgeClient, directory: string) {
...
function defaultModelFromConfig(
  ...
  // a default. Configured model, webforge provider, then sorted best model keep
  // the protocol response deterministic without extra session/message reads.
  const webforgeProvider = providers[ProviderV2.ID.make("webforge")]
  const webforgeModel = webforgeProvider ? Provider.sort(Object.values(webforgeProvider.models))[0] : undefined
  if (webforgeProvider && webforgeModel) return { providerID: webforgeProvider.id, modelID: webforgeModel.id }
...
function fromUnknownError(error: unknown, service?: string): Error {
  ...
  return new ACPError.ServiceFailureError({ safeMessage: "WebForge service failure", service })
}
