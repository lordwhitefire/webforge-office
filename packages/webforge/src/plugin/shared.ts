...
export const DEPRECATED_PLUGIN_PACKAGES = ["webforge-openai-codex-auth", "webforge-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, webforgeVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(webforgeVersion) || semver.major(webforgeVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.webforge
  if (typeof range !== "string") return
  if (!semver.satisfies(webforgeVersion, range)) {
    throw new Error(`Plugin requires webforge ${range} but running ${webforgeVersion}`)
  }
}
