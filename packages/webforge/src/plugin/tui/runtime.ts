...
import {
  ...
  type TuiSlotPlugin,
  type TuiTheme,
} from "@webforge-ai/plugin/tui"
import path from "path"
import { fileURLToPath } from "url"
import { TuiConfig } from "@/config/tui"
import { AppNodeBuilder } from "@webforge-ai/core/effect/app-node-builder"
import { errorData, errorMessage } from "@webforge-ai/tui/util/error"
...
import { hasTheme, upsertTheme } from "@webforge-ai/tui/context/theme"
import { Global } from "@webforge-ai/core/global"
import { Filesystem } from "@/util/filesystem"
import { Process } from "@/util/process"
import { Flock } from "@webforge-ai/core/util/flock"
import { Flag } from "@webforge-ai/core/flag/flag"
import { internalTuiPlugins, type InternalTuiPlugin } from "./internal"
import type { HostPluginApi, HostSlots } from "@webforge-ai/tui/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@webforge-ai/core/v1/config/plugin"
import { createCommandShim } from "@webforge-ai/tui/plugin/command-shim"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@webforge-ai/tui/plugin/runtime"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".webforge"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".webforge", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".webforge", "tui.json"),
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.WEBFORGE_PURE ? [] : pluginOrigins
    if (Flag.WEBFORGE_PURE && pluginOrigins.length) {
    }
