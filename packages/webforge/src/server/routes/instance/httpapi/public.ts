...
import { WebForgeHttpApi } from "./api"
...
const LegacyComponentDescriptions: Record<string, string> = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for webforge serve and web commands",
...
function fixSelfReferencingComponents(spec: OpenApiSpec) {
  ...
  const raw: OpenApiSpec = OpenApi.fromApi(WebForgeHttpApi)
...
export const PublicApi = WebForgeHttpApi.annotateMerge(
  OpenApi.annotations({
    title: "webforge",
    version: "1.0.0",
    description: "webforge api",
