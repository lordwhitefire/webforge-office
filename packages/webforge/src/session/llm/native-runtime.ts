...
import type { LLMClientShape } from "@webforge-ai/llm/route"
...
function statusWithFetch(
  ...
  if (providerID !== "openai" && providerID !== "anthropic" && !providerID.startsWith("webforge"))
    return { type: "unsupported", reason: "provider is not openai, webforge, or anthropic" }
...
export function stream(input: StreamInput): StreamResult {
  ...
  // Integration point with @webforge-ai/llm: native-request lowers session data
...
export function nativeTools(tools: Record<string, Tool>, input: Pick<StreamInput, "messages" | "abort">) {
  return Object.fromEntries(
    Object.entries(tools).map(([name, item]) => [
      name,
      // Tool execution remains webforge-owned. The native runtime only adapts
      // the @webforge-ai/llm tool call back into the AI SDK Tool.execute shape.
      NativeTool.make({
