...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from webforge.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const webforge = await webforgeFiles(input)
  for (const file of webforge) {
...
async function webforgeFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "webforge"),
    ...(await Filesystem.findUp(["webforge.json", "webforge.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "webforge"))
  }
  if (Flag.WEBFORGE_CONFIG) files.push(Flag.WEBFORGE_CONFIG)
