...
import { isRecord } from "@webforge-ai/tui/util/record"
...
import { TuiKeybind } from "@webforge-ai/tui/config/keybind"
...
import { TuiConfig } from "@webforge-ai/tui/config"
...
export class Service extends Context.Service<Service, Interface>()("@webforge/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old webforge.json shape) still get their settings applied.
  ...
  // Every config dir we may read from: global config dir, any `.webforge`
  // folders between cwd and home, and WEBFORGE_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.WEBFORGE_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit WEBFORGE_TUI_CONFIG override, if set.
  if (Flag.WEBFORGE_TUI_CONFIG) {
    const configFile = Flag.WEBFORGE_TUI_CONFIG
  ...
  // 4. `.webforge` directories (and WEBFORGE_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".webforge") || dir === Flag.WEBFORGE_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".webforge") && dir !== Flag.WEBFORGE_CONFIG_DIR) continue
