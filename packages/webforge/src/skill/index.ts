...
import { FrontmatterError } from "@webforge-ai/core/v1/config/error"
import { ConfigMarkdown } from "@/config/markdown"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Glob } from "@webforge-ai/core/util/glob"
...
const WEBFORGE_SKILL_PATTERN = "{skill,skills}/**/SKILL.md"
const SKILL_PATTERN = "**/SKILL.md"

// Built-in skill that ships with webforge. The model's intuition for what an
// webforge.json should look like is often wrong, and webforge hard-fails on
// invalid config, so users hit cryptic startup errors. Loading this skill
// when the model is asked to touch webforge's own config files gives it the
// actual schemas instead of guesses.
const CUSTOMIZE_WEBFORGE_SKILL_NAME = "customize-webforge"
const CUSTOMIZE_WEBFORGE_SKILL_DESCRIPTION =
  "Use ONLY when the user is editing or creating webforge's own configuration: webforge.json, webforge.jsonc, files under .webforge/, or files under ~/.config/webforge/. Also use when creating or fixing webforge agents, subagents, skills, plugins, MCP servers, or permission rules. Do not use for the user's own application code, or for any project that is not configuring webforge itself."
const CUSTOMIZE_WEBFORGE_SKILL_BODY = SkillPlugin.CustomizeWebforgeContent
...
const discoverSkills = Effect.fnUntraced(function* (
  ...
  for (const dir of configDirs) {
    yield* scan(state, dir, WEBFORGE_SKILL_PATTERN)
  }
...
export class Service extends Context.Service<Service, Interface>()("@webforge/Skill") {}
  ...
  Effect.gen(function* () {
    ...
    const state = yield* InstanceState.make(
      Effect.fn("Skill.state")(function* () {
        ...
        s.skills[CUSTOMIZE_WEBFORGE_SKILL_NAME] = {
          name: CUSTOMIZE_WEBFORGE_SKILL_NAME,
          description: CUSTOMIZE_WEBFORGE_SKILL_DESCRIPTION,
          location: "<built-in>",
          content: CUSTOMIZE_WEBFORGE_SKILL_BODY,
        }
