...
import { ModelsDev } from "@webforge-ai/core/models-dev"
import { HttpRecorder } from "@webforge-ai/http-recorder"
import { HttpRecorderInternal } from "@webforge-ai/http-recorder/internal"
...
import { LLMEvent, LLMResponse } from "@webforge-ai/llm"
import { RequestExecutor } from "@webforge-ai/llm/route"
...
const zenURL = (connection: string) => `https://console.webforge.ai/proxy/connections/${connection}/v1`
...
function decodeRecordOpenAIOAuth() {
  const value = process.env.WEBFORGE_RECORD_OPENAI_AUTH
...
const RECORDED_SCENARIOS = [
  {
    ...
    tags: ["webforge", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("WEBFORGE_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.openai,
        name: "OpenAI",
        ...
        model,
        options: {
          apiKey: envValue("WEBFORGE_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY") ?? "fixture-openai-key",
  ...
  },
  {
    ...
    protocol: "openai-responses",
    tags: ["webforge", "native", "oauth", "tool-loop"],
  ...
  },
  {
    id: "webforge-proxy",
    name: "WebForge proxy",
    providerID: ProviderV2.ID.webforge,
    modelID: "gpt-5.2-codex",
    cassette: "session/native-zen-tool-loop",
    protocol: "openai-responses",
    tags: ["webforge", "zen", "native", "tool-loop"],
    canRecord: () => Boolean(process.env.WEBFORGE_RECORD_CONSOLE_TOKEN && process.env.WEBFORGE_RECORD_ZEN_ORG_ID),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.webforge,
        name: "WebForge Zen",
        env: ["WEBFORGE_CONSOLE_TOKEN"],
        npm: "@ai-sdk/openai-compatible",
        api: zenURL(process.env.WEBFORGE_RECORD_ZEN_CONNECTION ?? "fixture"),
        model,
        options: {
          apiKey: process.env.WEBFORGE_RECORD_CONSOLE_TOKEN ?? "fixture-console-token",
          headers: { "x-org-id": process.env.WEBFORGE_RECORD_ZEN_ORG_ID ?? "fixture-org" },
        },
      }),
  },
  {
    ...
    tags: ["webforge", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("WEBFORGE_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.anthropic,
        name: "Anthropic",
        ...
        model,
        options: {
          apiKey: envValue("WEBFORGE_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY") ?? "fixture-anthropic-key",
...
const selectedScenarios = new Set(
  (envValue("WEBFORGE_RECORDED_SCENARIO", "RECORDED_PROVIDER") ?? "")
    .split(",")
...
const recordError = (scenario: RecordedScenario) =>
  scenario.id === "openai-oauth"
    ? "Set WEBFORGE_RECORD_OPENAI_AUTH to an OAuth auth JSON object in the recording environment."
...
function recordedNativeLLMLayer(scenario: RecordedScenario) {
  ...
  // Only the HTTP client is recorded; RequestExecutor and the webforge LLM stack remain real.
  const metadata = {
...
const writeConfig = (directory: string, scenario: RecordedScenario, model: ModelsDev.Provider["models"][string]) =>
  Effect.promise(() =>
    Bun.write(
      path.join(directory, "webforge.json"),
