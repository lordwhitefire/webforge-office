// Subprocess integration tests for `webforge run` (non-interactive mode).
// These exercise the real CLI binary against a TestLLMServer running in the
// same process. See `test/lib/cli-process.ts` for the harness — each test uses
// `webforge.run(message, opts?)` to spawn `bun src/index.ts run ...` with
// `WEBFORGE_CONFIG_CONTENT` providing the test provider config inline.
import { describe, expect } from "bun:test"
import { Effect } from "effect"
import { reply } from "../../lib/llm-server"
import { cliIt } from "../../lib/cli-process"

describe("webforge run (non-interactive subprocess)", () => {
  ...
  cliIt.concurrent(
    ...
    ({ llm, webforge }) =>
      Effect.gen(function* () {
        ...
        const result = yield* webforge.run("say hi")
        webforge.expectExit(result, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, webforge }) =>
      Effect.gen(function* () {
        ...
        const result = yield* webforge.run("use a tool", {
  ...
  cliIt.concurrent(
    ...
    ({ llm, webforge }) =>
      Effect.gen(function* () {
        ...
        const thinking = yield* webforge.run("think", { extraArgs: ["--thinking"] })
        webforge.expectExit(thinking, 0)
        expect(thinking.stdout).toBe("Thinking: considering\nanswer\n")

        yield* llm.reason("hidden", { text: "visible" })
        const plain = yield* webforge.run("think again")
        webforge.expectExit(plain, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, webforge }) =>
      Effect.gen(function* () {
        ...
        const result = yield* webforge.run("trigger midstream error", { timeoutMs: 30_000 })
  ...
  cliIt.concurrent(
    ...
    ({ llm, webforge }) =>
      Effect.gen(function* () {
        ...
        const result = yield* webforge.run("say hi", { format: "json" })
        webforge.expectExit(result, 0)

        const events = webforge.parseJsonEvents(result.stdout)
  ...
  cliIt.concurrent(
    ...
    ({ webforge }) =>
      Effect.gen(function* () {
        const result = yield* webforge.run("use an unknown model", {
          model: "test/nonexistent-model",
          format: "json",
        })
  ...
  cliIt.concurrent(
    ...
    ({ llm, webforge }) =>
      Effect.gen(function* () {
        ...
        const result = yield* webforge.run("exercise json records", {
          format: "json",
  ...
  cliIt.concurrent(
    ...
    ({ llm, webforge }) =>
      Effect.gen(function* () {
        ...
        const result = yield* webforge.run("fail after output", { format: "json" })
  ...
  cliIt.concurrent(
    ...
    ({ home, llm, webforge }) =>
      Effect.gen(function* () {
        ...
        const denied = yield* webforge.run("request permission", { permission: { bash: "ask" } })
        webforge.expectExit(denied, 0)
        ...
        const allowed = yield* webforge.run("request permission", {
          permission: { bash: "ask" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        webforge.expectExit(allowed, 0)
        ...
        const explicitlyDenied = yield* webforge.run("request denied permission", {
          permission: { bash: "deny" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        webforge.expectExit(explicitlyDenied, 0)
  ...
  cliIt.live(
    ...
    ({ home, llm, webforge }) =>
      Effect.gen(function* () {
        ...
        const server = yield* webforge.serve()

        const result = yield* webforge.run("read the attachment", {
  ...
  cliIt.concurrent(
    ...
    ({ home, webforge }) =>
      Effect.gen(function* () {
        const result = yield* webforge.run("read the directory", {
  ...
  cliIt.live(
    ...
    ({ llm, webforge }) =>
      Effect.gen(function* () {
        yield* llm.hang
        const run = yield* webforge.startRun("wait forever")
        yield* llm.wait(1)
        run.interrupt()
