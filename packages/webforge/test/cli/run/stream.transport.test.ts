...
import { WebforgeClient, type GlobalEvent } from "@webforge-ai/sdk/v2"
import { createSessionTransport } from "@/cli/cmd/run/stream.transport"
import type { FooterApi, FooterEvent, LocalReplayRow, RunFilePart, StreamCommit } from "@/cli/cmd/run/types"

type EventStream = Awaited<ReturnType<WebforgeClient["event"]["subscribe"]>>["stream"]
type GlobalEventStream = Awaited<ReturnType<WebforgeClient["global"]["event"]>>["stream"]
type SdkEvent = EventStream extends AsyncGenerator<infer T, unknown, unknown> ? T : never
type SessionMessage = NonNullable<Awaited<ReturnType<WebforgeClient["session"]["messages"]>>["data"]>[number]
type SessionChild = NonNullable<Awaited<ReturnType<WebforgeClient["session"]["children"]>>["data"]>[number]
type SessionToolPart = Extract<SessionMessage["parts"][number], { type: "tool" }>
type SessionStatusMap = NonNullable<Awaited<ReturnType<WebforgeClient["session"]["status"]>>["data"]>
...
function ok<T>(data: T) {
  return Promise.resolve({
    data,
    error: undefined,
    request: new Request("https://webforge.test"),
...
function sdk(
  input: {
    ...
    subscribe?: WebforgeClient["event"]["subscribe"]
    globalEvent?: WebforgeClient["global"]["event"]
    promptAsync?: WebforgeClient["session"]["promptAsync"]
    status?: WebforgeClient["session"]["status"]
    messages?: WebforgeClient["session"]["messages"]
    children?: WebforgeClient["session"]["children"]
    permissions?: WebforgeClient["permission"]["list"]
    questions?: WebforgeClient["question"]["list"]
  } = {},
) {
  const client = new WebforgeClient()

  const subscribe: WebforgeClient["event"]["subscribe"] = input.subscribe ?? (() => sse(input.stream ?? emptyStream()))
  const globalEvent: WebforgeClient["global"]["event"] =
    input.globalEvent ?? (() => globalSse(input.globalStream ?? wrapGlobalStream(input.stream ?? emptyStream())))
  const promptAsync: WebforgeClient["session"]["promptAsync"] = input.promptAsync ?? (() => ok(undefined))
  const status: WebforgeClient["session"]["status"] = input.status ?? (() => ok({}))
  const messages: WebforgeClient["session"]["messages"] = input.messages ?? (() => ok([]))
  const children: WebforgeClient["session"]["children"] = input.children ?? (() => ok([]))
  const permissions: WebforgeClient["permission"]["list"] = input.permissions ?? (() => ok([]))
  const questions: WebforgeClient["question"]["list"] = input.questions ?? (() => ok([]))
