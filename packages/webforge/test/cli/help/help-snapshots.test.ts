...
// terminal sizes. The default webforge tui command is excluded —
// `webforge --help` includes an ASCII banner that pulls in the install
...
// Top-level commands. Order matches what `webforge --help` prints today;
...
// top-level help on `--help` and exits 1; not a real webforge command.
const TOP_LEVEL = [
  "acp",
  "mcp",
  "attach",
...
describe("webforge CLI help-text snapshots", () => {
  ...
  cliIt.live(
    ...
    ({ webforge }) =>
      Effect.gen(function* () {
        const topLevel = yield* webforge.spawn(["--help"], { env: SNAPSHOT_ENV })
        ...
        const [failures, results] = yield* Effect.partition(
          ...
          (argv) =>
            Effect.gen(function* () {
              const result = yield* webforge.spawn([...argv, "--help"], { env: SNAPSHOT_ENV })
              if (result.exitCode !== 0) {
                return yield* Effect.fail(`webforge ${argv.join(" ")}: exit ${result.exitCode}`)
              }
              return { argv, result }
            }),
          { concurrency: 8 },
        ...
        for (const { argv, result } of results) {
          ...
          expect(normalize(result.stderr)).toMatchSnapshot(`webforge ${argv.join(" ")} --help`)
        }
