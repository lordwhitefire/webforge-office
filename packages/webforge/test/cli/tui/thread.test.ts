...
describe("tui thread", () => {
  ...
  cliIt.live("rejects mini-only options without --mini", ({ webforge }) =>
    Effect.gen(function* () {
      const result = yield* webforge.spawn(["--replay-limit", "10"])

      webforge.expectExit(result, 1)
      expect(result.stderr).toContain("--replay-limit requires --mini")
    }),
  )

  cliIt.live("routes attached sessions to mini mode", ({ webforge }) =>
    Effect.gen(function* () {
      const result = yield* webforge.spawn(["attach", "http://127.0.0.1:1", "--mini"])
  ...
  cliIt.live("rejects network options in mini mode", ({ webforge }) =>
    Effect.gen(function* () {
      const result = yield* webforge.spawn(["--mini", "--port", "4096"])
