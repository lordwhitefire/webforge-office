...
describe("webforge read-only commands (smoke)", () => {
  // `mcp list` reads MCP server config and pings each one. With the empty
  // WEBFORGE_CONFIG_CONTENT={} we provide, no servers should be configured
  // and the command should report that cleanly.
  cliIt.live(
    "mcp list: exits 0",
    ({ webforge }) =>
      Effect.gen(function* () {
        const r = yield* webforge.spawn(["mcp", "list"])
        webforge.expectExit(r, 0, "mcp list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ webforge }) =>
      Effect.gen(function* () {
        const r = yield* webforge.spawn(["providers", "list"])
        webforge.expectExit(r, 0, "providers list")
  ...
  cliIt.live(
    ...
    ({ webforge }) =>
      Effect.gen(function* () {
        const r = yield* webforge.spawn(["models"])
        webforge.expectExit(r, 0, "models")
  ...
  cliIt.live(
    ...
    ({ webforge }) =>
      Effect.gen(function* () {
        const r = yield* webforge.spawn(["agent", "list"])
        webforge.expectExit(r, 0, "agent list")
      }),
    60_000,
  )

  // `session list` reads the session DB. Fresh WEBFORGE_TEST_HOME means
  // empty DB. Exit 0 with no sessions.
  cliIt.live(
    "session list: exits 0",
    ({ webforge }) =>
      Effect.gen(function* () {
        const r = yield* webforge.spawn(["session", "list"])
        webforge.expectExit(r, 0, "session list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ webforge }) =>
      Effect.gen(function* () {
        const r = yield* webforge.spawn(["stats"])
        webforge.expectExit(r, 0, "stats")
      }),
    60_000,
  )
  ...
  // in production it'd be a path under WEBFORGE_TEST_HOME / XDG_DATA_HOME.
  // Accept either form — both prove the resolver ran without crashing.
  cliIt.live(
    "db path: exits 0 and prints a path or :memory:",
    ({ webforge }) =>
      Effect.gen(function* () {
        const r = yield* webforge.spawn(["db", "path"])
        webforge.expectExit(r, 0, "db path")
