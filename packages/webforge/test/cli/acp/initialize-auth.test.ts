...
describe("webforge acp initialize/auth subprocess", () => {
  cliIt.live(
    "initialize responds with capabilities",
    ({ webforge }) =>
      Effect.gen(function* () {
        const initialized = yield* initialize(yield* createAcpClient({ webforge }))
        ...
        expect(initialized.agentInfo?.name).toBe("WebForge")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ webforge }) =>
      Effect.gen(function* () {
        const acp = yield* createAcpClient({ webforge })
        const initialized = yield* initialize(acp)

        expect(initialized.authMethods?.[0]?.id).toBe("webforge-login")
        expect(initialized.authMethods?.[0]?._meta?.["terminal-auth"]).toBeDefined()
        expect(yield* acp.request<AuthenticateResponse>("authenticate", { methodId: "webforge-login" })).toMatchObject({
          result: {},
        })
        ...
        expect(JSON.stringify(rejected.error)).not.toContain(process.env.WEBFORGE_AUTH_CONTENT ?? "not-present")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ webforge }) =>
      Effect.gen(function* () {
        ...
        expect(initialized.result?.authMethods?.[0]?.id).toBe("webforge-login")
