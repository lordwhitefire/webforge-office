...
export function createAcpClient(input: Pick<CliFixture, "webforge">, env?: Record<string, string>) {
  return Effect.gen(function* () {
    return createJsonRpcAcpClient(yield* input.webforge.acp(env ? { env } : undefined))
  })
}
  ...
  return Effect.gen(function* () {
    return expectOk(
      yield* acp.request<InitializeResponse>("initialize", {
        ...
        clientInfo: { name: "webforge-local-acp", version: "0.1.0" },
      }),
    )
  })
}
