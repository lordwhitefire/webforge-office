// Subprocess test harness for the webforge CLI. Spawns the real binary against
...
//
// Configuration flows through webforge's built-in test affordances:
//   - WEBFORGE_CONFIG_CONTENT      : provider config inline, no files to find
//   - WEBFORGE_TEST_HOME           : pins os.homedir() → tmpdir
//   - WEBFORGE_DISABLE_PROJECT_CONFIG : skip walking up for webforge.json
//   - WEBFORGE_PURE                : skip external plugin discovery + install
//   - WEBFORGE_DISABLE_AUTOUPDATE / AUTOCOMPACT / MODELS_FETCH : no background work
// Plus HOME / XDG_* pointing at the tmpdir for belt-and-suspenders isolation.
//
// Today only `webforge.run` is fully wired. The shape supports adding more
// builders (`webforge.serve(opts)`, `webforge.acp(opts)`, `webforge.auth(...)`)
// without changing the fixture. Long-lived commands like `serve` will need a
// different return shape — see the TODO at the bottom of WebforgeCli.
...
import { AppProcess } from "@webforge-ai/core/process"
...
const webforgeRoot = path.resolve(import.meta.dir, "../../")
const cliEntry = path.join(webforgeRoot, "src/index.ts")
...
function isolatedEnv(home: string, configJson: string): Record<string, string> {
  return {
    WEBFORGE_TEST_HOME: home,
    HOME: home,
    XDG_CONFIG_HOME: path.join(home, ".config"),
    XDG_DATA_HOME: path.join(home, ".local/share"),
    XDG_STATE_HOME: path.join(home, ".local/state"),
    XDG_CACHE_HOME: path.join(home, ".cache"),
    WEBFORGE_CONFIG_CONTENT: configJson,
    WEBFORGE_DISABLE_PROJECT_CONFIG: "1",
    WEBFORGE_PURE: "1",
    WEBFORGE_DISABLE_AUTOUPDATE: "1",
    WEBFORGE_DISABLE_AUTOCOMPACT: "1",
    WEBFORGE_DISABLE_MODELS_FETCH: "1",
    WEBFORGE_AUTH_CONTENT: "{}",
  }
}
...
// Typed equivalent of constructing argv for `webforge run`. New flags should
...
// `webforge serve` is a long-lived process — it never exits on its own.
...
// `webforge acp` speaks newline-delimited JSON-RPC over stdin/stdout. It is
...
export type WebforgeCli = {
  ...
  // Spawn `webforge serve` and wait until it's listening. Long-lived: the
  ...
  // Spawn `webforge acp` and return a duplex JSON-RPC handle. Long-lived:
...
export function withCliFixture<A, E>(
  ...
  return Effect.gen(function* () {
    ...
    const spawn = Effect.fn("webforge.spawn")(function* (args: string[], opts?: SpawnOpts) {
    ...
    const runOpts = (opts?: RunOpts): SpawnOpts | undefined => {
      ...
      return {
        ...
        env: {
          ...opts.env,
          WEBFORGE_CONFIG_CONTENT: JSON.stringify({
    ...
    const startRun = Effect.fn("webforge.startRun")(function* (message: string, opts?: RunOpts) {
      const start = Date.now()
    ...
    const serve = Effect.fn("webforge.serve")(function* (opts?: ServeOpts) {
      const argv = ["serve"]
      ...
      //   "webforge server listening on http://<host>:<port>"
      ...
      const match = yield* Deferred.await(readyDeferred).pipe(
        Effect.timeoutOrElse({
          ...
          orElse: () =>
            Effect.fail(
              new Error(
                `webforge serve did not become ready within ${readyTimeoutMs}ms\n` +
    ...
    const acp = Effect.fn("webforge.acp")(function* (opts?: AcpOpts) {
      const argv = ["acp"]
    ...
    const webforge: WebforgeCli = { run, startRun, serve, acp, spawn, expectExit, parseJsonEvents }

    return yield* fn({ llm, home, webforge })
    // FetchHttpClient is provided so test bodies can `yield* HttpClient.HttpClient`
    // and hit endpoints on `webforge.serve()` without rolling their own fetch.
  }).pipe(
    Effect.provide(
      Layer.mergeAll(
        TestLLMServer.layer,
...
function expectExit(result: RunResult, expected: number, label = "webforge") {
...
// every call site. Use this for any test that needs the webforge CLI fixture.
//
...
// resources (e.g. `webforge.serve`) without an extra `Effect.scoped` wrapper —
