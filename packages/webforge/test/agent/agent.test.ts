...
it.instance("plan agent denies edits except .webforge/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("edit", ".webforge/plans/foo.md", plan!.permission).action).toBe("allow")
  }),
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const skillDir = path.join(test.directory, ".webforge", "skill", "perm-skill")
...
`,
      ...
      const home = process.env.WEBFORGE_TEST_HOME
      process.env.WEBFORGE_TEST_HOME = test.directory
