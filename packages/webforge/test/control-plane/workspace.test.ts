...
import { EventSequenceTable } from "@webforge-ai/core/event/sql"
...
import { WorkspaceV2 } from "@webforge-ai/core/workspace"
import { WorkspaceTable } from "@webforge-ai/core/control-plane/workspace.sql"
...
const originalEnv = {
  WEBFORGE_AUTH_CONTENT: process.env.WEBFORGE_AUTH_CONTENT,
  WEBFORGE_EXPERIMENTAL_WORKSPACES: process.env.WEBFORGE_EXPERIMENTAL_WORKSPACES,
...
beforeEach(() => {
  restoreEnv()
  process.env.WEBFORGE_EXPERIMENTAL_WORKSPACES = "true"
})
...
async function initGitRepo(dir: string) {
  ...
  await $`git config user.email "test@webforge.test"`.cwd(dir).quiet()
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const instance = yield* requireInstance
        const workspace = yield* Workspace.Service
        process.env.WEBFORGE_AUTH_CONTENT = JSON.stringify({ test: { type: "api", key: "secret" } })
        process.env.OTEL_EXPORTER_OTLP_HEADERS = "authorization=otel"
        process.env.OTEL_EXPORTER_OTLP_ENDPOINT = "https://otel.test"
        process.env.OTEL_RESOURCE_ATTRIBUTES = "service.name=webforge-test"
        ...
        expect(JSON.parse(recorded.calls.create[0].env.WEBFORGE_AUTH_CONTENT ?? "{}")).toEqual({
          test: { type: "api", key: "secret" },
        })
        expect(recorded.calls.create[0].env.WEBFORGE_WORKSPACE_ID).toBe(workspaceID)
        expect(recorded.calls.create[0].env.WEBFORGE_EXPERIMENTAL_WORKSPACES).toBe("true")
        ...
        expect(recorded.calls.create[0].env.OTEL_RESOURCE_ATTRIBUTES).toBe("service.name=webforge-test")
