...
describe("util.process", () => {
  ...
  test("merges environment overrides", async () => {
    const out = await Process.run(node('process.stdout.write(process.env.WEBFORGE_TEST ?? "")'), {
      env: {
        WEBFORGE_TEST: "set",
      },
    })
  ...
  test("uses shell in run on Windows", async () => {
    ...
    const out = await Process.run(["set", "WEBFORGE_TEST_SHELL"], {
      shell: true,
      env: {
        WEBFORGE_TEST_SHELL: "ok",
      },
    })

    expect(out.code).toBe(0)
    expect(out.stdout.toString()).toContain("WEBFORGE_TEST_SHELL=ok")
  })
