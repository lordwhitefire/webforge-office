...
import type { Event, WebforgeClient } from "@webforge-ai/sdk/v2"
...
type PermissionReplyParams = Parameters<WebforgeClient["permission"]["reply"]>[0]
...
async function tempFile(name: string, content: string) {
  const dir = await mkdtemp(path.join(tmpdir(), "webforge-acp-permission-"))
