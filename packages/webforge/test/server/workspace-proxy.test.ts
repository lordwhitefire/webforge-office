...
describe("HttpApi workspace proxy", () => {
  ...
  it.live("strips webforge-internal headers and merges extra headers", () =>
    Effect.gen(function* () {
      ...
      expect(forwarded["x-webforge-directory"]).toBeUndefined()
      expect(forwarded["x-webforge-workspace"]).toBeUndefined()
