 ...
 *
 * Memory is stored as markdown files in <project>/.webforge/memory/.
...
export default {
  ...
  async execute(args, context) {
    ...
    if (!isWebForgeAgent(fs, path, callerAgent, projectPath)) {
      return `BLOCKED: ${callerAgent} is not a registered WebForge agent. memory only applies to WebForge agents.`
    }

    const memoryDir = path.join(projectPath, ".webforge", "memory")
